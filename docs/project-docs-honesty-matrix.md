...
| Test architecture | `config/test-source-size-baseline.txt`, command part manifest, shared fixture, suite scanner | `test_architecture_contract` rejects assertion loss, missing calls, duplicate parts, unbaselined files above 1,000 nonblank lines and baseline growth | command monolith repaid in v1665; one shard-readiness baseline remains for v1666 |

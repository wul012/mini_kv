...
This index is the K5 retention ledger for the path-stable archive roots in this repository. It records the original v1620 baseline, the v1657 post-OSFS remeasurement, the v1658 track-review evidence closeout, the bounded v1659-v1662 elegance evidence, the selective v1663 OSFS teaching evidence, the v1664 README exhibit proof, the v1665-v1667 test/build-architecture proof, the v1668-v1669 receipt-profile evidence, the selective v1670 atomic-write evidence, and the v1671 test-link topology evidence.
...
| `f/` | 1597-1671 | 380 | 11,709,146 | 11.17 | 128 | ACTIVE ROOT; EXISTING VERSION DIRECTORIES FROZEN |

Latest measured version archive:

| Version archive | Files | Bytes | MiB | Budget MiB | State |
|---|---:|---:|---:|---:|---|
| `f/1671/` | 6 | 76,288 | 0.07 | 8 | within budget |

The table above is the direct output of `python scripts/archive_inventory.py --budget-mib 8 --strict` on 2026-07-20 after the six selective v1671 evidence files were written. The script emitted `warnings: none`; no historical path was moved or rewritten. Earlier inventories remain frozen in their own summaries. v1671 keeps four selected topology/compatibility/full-CTest/CLI screenshots, one explanation and one text summary; it does not retain build binaries, the raw 700-line CTest output, smoke files or the temporary evidence page.

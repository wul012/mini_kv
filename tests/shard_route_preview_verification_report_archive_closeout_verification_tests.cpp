...
int main() {
    ...
    assert_contains(verification, "\"verificationStage\":\"route-preview-verification-report-archive-closeout-verification-source-closeout-index-audit\"");
    assert_contains(verification, "\"verificationStageSequence\":9");
    assert_contains(verification, "\"verificationReleaseVersion\":\"v389\"");
    assert_contains(verification, "\"publishedStageCount\":9");
    assert_contains(verification, "\"plannedStageCount\":20");
    assert_contains(verification, "\"sourceFrozenReleaseVersion\":\"v388\"");
    assert_contains(verification, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v388.json\"");

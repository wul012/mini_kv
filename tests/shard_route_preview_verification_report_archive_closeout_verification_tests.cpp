...
int main() {
    ...
    assert_contains(verification, "\"verificationStage\":\"route-preview-verification-report-archive-closeout-verification-consumer-handoff-audit\"");
    assert_contains(verification, "\"verificationStageSequence\":13");
    assert_contains(verification, "\"verificationReleaseVersion\":\"v393\"");
    assert_contains(verification, "\"publishedStageCount\":13");
    assert_contains(verification, "\"plannedStageCount\":20");
    assert_contains(verification, "\"sourceFrozenReleaseVersion\":\"v392\"");
    assert_contains(verification, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v392.json\"");

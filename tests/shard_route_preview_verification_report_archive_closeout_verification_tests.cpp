...
int main() {
    ...
    assert_contains(verification, "\"verificationStage\":\"route-preview-verification-report-archive-closeout-verification-boundary-field-audit\"");
    assert_contains(verification, "\"verificationStageSequence\":16");
    assert_contains(verification, "\"verificationReleaseVersion\":\"v396\"");
    assert_contains(verification, "\"publishedStageCount\":16");
    assert_contains(verification, "\"plannedStageCount\":20");
    assert_contains(verification, "\"sourceFrozenReleaseVersion\":\"v395\"");
    assert_contains(verification, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v395.json\"");

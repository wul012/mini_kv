...
int main() {
    ...
    assert_contains(verification, "\"verificationStage\":\"route-preview-verification-report-archive-closeout-verification-command-contract\"");
    assert_contains(verification, "\"verificationStageSequence\":1");
    assert_contains(verification, "\"verificationReleaseVersion\":\"v381\"");
    assert_contains(verification, "\"publishedStageCount\":1");
    assert_contains(verification, "\"plannedStageCount\":20");
    assert_contains(verification, "\"sourceFrozenReleaseVersion\":\"v380\"");
    assert_contains(verification, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v380.json\"");

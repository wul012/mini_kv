...
int main() {
    ...
    assert_contains(verification, "\"verificationStage\":\"route-preview-verification-report-archive-closeout-verification-catalog-alignment\"");
    assert_contains(verification, "\"verificationStageSequence\":2");
    assert_contains(verification, "\"verificationReleaseVersion\":\"v382\"");
    assert_contains(verification, "\"publishedStageCount\":2");
    assert_contains(verification, "\"plannedStageCount\":20");
    assert_contains(verification, "\"sourceFrozenReleaseVersion\":\"v381\"");
    assert_contains(verification, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v381.json\"");

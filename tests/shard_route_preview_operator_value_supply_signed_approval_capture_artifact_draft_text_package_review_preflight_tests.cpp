...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageReviewPreflightStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-separate-reviewer-control\"");
    assert_contains(json, "\"draftTextPackageReviewPreflightReleaseVersion\":\"v805\"");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v804\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v804.json\"");
    ...
    assert_contains(json, "\"preparedReviewCriterionCount\":20");
    ...
    assert_contains(json, "\"criterionCode\":\"separate_reviewer_control\"");
    assert_contains(json, "\"controlCode\":\"separate_reviewer_control_control\"");
    assert_contains(json, "\"profileCode\":\"draft_text_package_review_preflight_separate_reviewer_control_profile\"");
    ...
    assert(digest.find("v805-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-separate-reviewer-control") != std::string::npos);
    assert(preflight::published_stage_count() == 20);
}

...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageReviewPreflightStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-signature-redaction-criterion\"");
    assert_contains(json, "\"draftTextPackageReviewPreflightReleaseVersion\":\"v795\"");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v794\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v794.json\"");
    ...
    assert_contains(json, "\"preparedReviewCriterionCount\":10");
    ...
    assert_contains(json, "\"criterionCode\":\"signature_redaction_criterion\"");
    assert_contains(json, "\"controlCode\":\"signature_redaction_criterion_control\"");
    assert_contains(json, "\"profileCode\":\"draft_text_package_review_preflight_signature_redaction_criterion_profile\"");
    ...
    assert(digest.find("v795-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-signature-redaction-criterion") != std::string::npos);
    assert(preflight::published_stage_count() == 10);
}

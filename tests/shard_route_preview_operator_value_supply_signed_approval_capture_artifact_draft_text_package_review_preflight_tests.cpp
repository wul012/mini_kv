...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageReviewPreflightStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-artifact-digest-binding-criterion\"");
    assert_contains(json, "\"draftTextPackageReviewPreflightReleaseVersion\":\"v790\"");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v789\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v789.json\"");
    ...
    assert_contains(json, "\"preparedReviewCriterionCount\":5");
    ...
    assert_contains(json, "\"criterionCode\":\"artifact_digest_binding_criterion\"");
    assert_contains(json, "\"controlCode\":\"artifact_digest_binding_criterion_control\"");
    assert_contains(json, "\"profileCode\":\"draft_text_package_review_preflight_artifact_digest_binding_criterion_profile\"");
    ...
    assert(digest.find("v790-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-artifact-digest-binding-criterion") != std::string::npos);
    assert(preflight::published_stage_count() == 5);
}

...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageReviewPreflightStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-request-correlation-criterion\"");
    assert_contains(json, "\"draftTextPackageReviewPreflightReleaseVersion\":\"v789\"");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v788\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v788.json\"");
    ...
    assert_contains(json, "\"preparedReviewCriterionCount\":4");
    ...
    assert_contains(json, "\"criterionCode\":\"request_correlation_criterion\"");
    assert_contains(json, "\"controlCode\":\"request_correlation_criterion_control\"");
    assert_contains(json, "\"profileCode\":\"draft_text_package_review_preflight_request_correlation_criterion_profile\"");
    ...
    assert(digest.find("v789-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-request-correlation-criterion") != std::string::npos);
    assert(preflight::published_stage_count() == 4);
}

...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageReviewPreflightStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-value-shape-criterion\"");
    assert_contains(json, "\"draftTextPackageReviewPreflightReleaseVersion\":\"v802\"");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v801\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v801.json\"");
    ...
    assert_contains(json, "\"preparedReviewCriterionCount\":17");
    ...
    assert_contains(json, "\"criterionCode\":\"value_shape_criterion\"");
    assert_contains(json, "\"controlCode\":\"value_shape_criterion_control\"");
    assert_contains(json, "\"profileCode\":\"draft_text_package_review_preflight_value_shape_criterion_profile\"");
    ...
    assert(digest.find("v802-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-value-shape-criterion") != std::string::npos);
    assert(preflight::published_stage_count() == 17);
}

...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageReviewPreflightStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-unreviewable-package-rejection-control\"");
    assert_contains(json, "\"draftTextPackageReviewPreflightReleaseVersion\":\"v806\"");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v805\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v805.json\"");
    ...
    assert_contains(json, "\"preparedReviewCriterionCount\":21");
    ...
    assert_contains(json, "\"criterionCode\":\"unreviewable_package_rejection_control\"");
    assert_contains(json, "\"controlCode\":\"unreviewable_package_rejection_control_control\"");
    assert_contains(json, "\"profileCode\":\"draft_text_package_review_preflight_unreviewable_package_rejection_control_profile\"");
    ...
    assert(digest.find("v806-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-unreviewable-package-rejection-control") != std::string::npos);
    assert(preflight::published_stage_count() == 21);
}

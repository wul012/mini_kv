...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageReviewPreflightStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-template-digest-binding-criterion\"");
    assert_contains(json, "\"draftTextPackageReviewPreflightReleaseVersion\":\"v791\"");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v790\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v790.json\"");
    ...
    assert_contains(json, "\"preparedReviewCriterionCount\":6");
    ...
    assert_contains(json, "\"criterionCode\":\"template_digest_binding_criterion\"");
    assert_contains(json, "\"controlCode\":\"template_digest_binding_criterion_control\"");
    assert_contains(json, "\"profileCode\":\"draft_text_package_review_preflight_template_digest_binding_criterion_profile\"");
    ...
    assert(digest.find("v791-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-template-digest-binding-criterion") != std::string::npos);
    assert(preflight::published_stage_count() == 6);
}

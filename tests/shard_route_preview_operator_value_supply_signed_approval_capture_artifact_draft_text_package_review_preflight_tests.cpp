...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageReviewPreflightStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-instruction-preflight-digest-criterion\"");
    assert_contains(json, "\"draftTextPackageReviewPreflightReleaseVersion\":\"v787\"");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v786\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v786.json\"");
    ...
    assert_contains(json, "\"preparedReviewCriterionCount\":2");
    ...
    assert_contains(json, "\"criterionCode\":\"instruction_preflight_digest_criterion\"");
    assert_contains(json, "\"controlCode\":\"instruction_preflight_digest_criterion_control\"");
    assert_contains(json, "\"profileCode\":\"draft_text_package_review_preflight_instruction_preflight_digest_criterion_profile\"");
    ...
    assert(digest.find("v787-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-instruction-preflight-digest-criterion") != std::string::npos);

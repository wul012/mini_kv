...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageReviewPreflightStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-redaction-policy-criterion\"");
    assert_contains(json, "\"draftTextPackageReviewPreflightReleaseVersion\":\"v803\"");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v802\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v802.json\"");
    ...
    assert_contains(json, "\"preparedReviewCriterionCount\":18");
    ...
    assert_contains(json, "\"criterionCode\":\"redaction_policy_criterion\"");
    assert_contains(json, "\"controlCode\":\"redaction_policy_criterion_control\"");
    assert_contains(json, "\"profileCode\":\"draft_text_package_review_preflight_redaction_policy_criterion_profile\"");
    ...
    assert(digest.find("v803-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-redaction-policy-criterion") != std::string::npos);
    assert(preflight::published_stage_count() == 18);
}

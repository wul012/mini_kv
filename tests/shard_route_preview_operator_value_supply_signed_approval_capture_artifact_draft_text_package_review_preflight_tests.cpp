...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageReviewPreflightStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-source-release-handle-criterion\"");
    assert_contains(json, "\"draftTextPackageReviewPreflightReleaseVersion\":\"v796\"");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v795\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v795.json\"");
    ...
    assert_contains(json, "\"preparedReviewCriterionCount\":11");
    ...
    assert_contains(json, "\"criterionCode\":\"source_release_handle_criterion\"");
    assert_contains(json, "\"controlCode\":\"source_release_handle_criterion_control\"");
    assert_contains(json, "\"profileCode\":\"draft_text_package_review_preflight_source_release_handle_criterion_profile\"");
    ...
    assert(digest.find("v796-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-source-release-handle-criterion") != std::string::npos);
    assert(preflight::published_stage_count() == 11);
}

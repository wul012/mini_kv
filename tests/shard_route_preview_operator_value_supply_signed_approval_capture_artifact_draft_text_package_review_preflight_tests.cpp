...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageReviewPreflightStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-runtime-payload-rejection-control\"");
    assert_contains(json, "\"draftTextPackageReviewPreflightReleaseVersion\":\"v807\"");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v806\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v806.json\"");
    ...
    assert_contains(json, "\"preparedReviewCriterionCount\":22");
    ...
    assert_contains(json, "\"criterionCode\":\"runtime_payload_rejection_control\"");
    assert_contains(json, "\"controlCode\":\"runtime_payload_rejection_control_control\"");
    assert_contains(json, "\"profileCode\":\"draft_text_package_review_preflight_runtime_payload_rejection_control_profile\"");
    ...
    assert(digest.find("v807-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-runtime-payload-rejection-control") != std::string::npos);
    assert(preflight::published_stage_count() == 22);
}

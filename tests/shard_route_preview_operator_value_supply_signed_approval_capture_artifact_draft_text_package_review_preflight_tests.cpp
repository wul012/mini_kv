...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageReviewPreflightStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-source-file-handle-criterion\"");
    assert_contains(json, "\"draftTextPackageReviewPreflightReleaseVersion\":\"v797\"");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v796\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v796.json\"");
    ...
    assert_contains(json, "\"preparedReviewCriterionCount\":12");
    ...
    assert_contains(json, "\"criterionCode\":\"source_file_handle_criterion\"");
    assert_contains(json, "\"controlCode\":\"source_file_handle_criterion_control\"");
    assert_contains(json, "\"profileCode\":\"draft_text_package_review_preflight_source_file_handle_criterion_profile\"");
    ...
    assert(digest.find("v797-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-source-file-handle-criterion") != std::string::npos);
    assert(preflight::published_stage_count() == 12);
}

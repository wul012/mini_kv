...
int main() {
    ...
    assert_contains(manifest, "\"project_version\":\"0.67.0\"");
    assert_contains(manifest, "\"release_version\":\"v67\"");
    ...
    assert_contains(manifest, "\"consumer_hint\":\"Node v167 rollback execution preflight contract\"");

    assert_contains(manifest, "\"command\":\"cmake -S . -B cmake-build-v67");
    assert_contains(manifest, "\"command\":\"cmake --build cmake-build-v67 --parallel 2\"");
    assert_contains(manifest, "\"command\":\"ctest --test-dir cmake-build-v67 --output-on-failure\"");
    ...
    assert_contains(manifest, "\"CHECKJSON LOAD data/dry-run-restore.snap\"");
    assert_contains(manifest, "\"CHECKJSON COMPACT\"");
    assert_contains(manifest, "\"CHECKJSON SETNXEX dryrun:token 30 value\"");
    assert_contains(manifest, "\"STORAGEJSON\"");
    assert_contains(manifest, "\"HEALTH\"");
    assert_contains(manifest, "\"GET dryrun:token\"");
    ...
    assert_contains(manifest, "\"cmake_project_version\":\"0.67.0\"");
    ...
    assert_contains(cmake_lists, "project(mini_kv VERSION 0.67.0");
    ...
    assert_contains(result.response, "\"version\":\"0.67.0\"");

...
int main() {
    ...
    assert_contains(manifest, "\"project_version\":\"0.64.0\"");
    assert_contains(manifest, "\"release_version\":\"v64\"");
    ...
    assert_contains(manifest, "\"consumer_hint\":\"Node v163 release rollback readiness runbook\"");

    assert_contains(manifest, "\"command\":\"cmake -S . -B cmake-build-v64");
    assert_contains(manifest, "\"command\":\"cmake --build cmake-build-v64 --parallel 2\"");
    assert_contains(manifest, "\"command\":\"ctest --test-dir cmake-build-v64 --output-on-failure\"");
    ...
    assert_contains(manifest, "\"CHECKJSON LOAD data/rollback.snap\"");
    assert_contains(manifest, "\"CHECKJSON COMPACT\"");
    assert_contains(manifest, "\"CHECKJSON SETNXEX rollback:token 30 value\"");
    assert_contains(manifest, "\"STORAGEJSON\"");
    assert_contains(manifest, "\"HEALTH\"");
    assert_contains(manifest, "\"GET rollback:token\"");
    ...
    assert_contains(manifest, "\"cmake_project_version\":\"0.64.0\"");
    ...
    assert_contains(cmake_lists, "project(mini_kv VERSION 0.64.0");
    ...
    assert_contains(result.response, "\"version\":\"0.64.0\"");

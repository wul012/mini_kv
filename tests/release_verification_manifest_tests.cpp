...
int main() {
    ...
    assert_contains(manifest, "\"project_version\":\"0.65.0\"");
    assert_contains(manifest, "\"release_version\":\"v65\"");
    ...
    assert_contains(manifest, "\"consumer_hint\":\"Node v164 cross-project release bundle gate\"");

    assert_contains(manifest, "\"command\":\"cmake -S . -B cmake-build-v65");
    assert_contains(manifest, "\"command\":\"cmake --build cmake-build-v65 --parallel 2\"");
    assert_contains(manifest, "\"command\":\"ctest --test-dir cmake-build-v65 --output-on-failure\"");
    ...
    assert_contains(manifest, "\"COMMANDSJSON\"");
    assert_contains(manifest, "\"CHECKJSON LOAD data/bundle.snap\"");
    assert_contains(manifest, "\"CHECKJSON COMPACT\"");
    assert_contains(manifest, "\"CHECKJSON SETNXEX bundle:token 30 value\"");
    assert_contains(manifest, "\"STORAGEJSON\"");
    assert_contains(manifest, "\"HEALTH\"");
    assert_contains(manifest, "\"GET bundle:token\"");
    ...
    assert_contains(manifest, "\"cmake_project_version\":\"0.65.0\"");
    ...
    assert_contains(cmake_lists, "project(mini_kv VERSION 0.65.0");
    ...
    assert_contains(result.response, "\"version\":\"0.65.0\"");

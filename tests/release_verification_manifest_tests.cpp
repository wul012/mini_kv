...
int main() {
    ...
    assert_contains(manifest, "\"project_version\":\"0.68.0\"");
    assert_contains(manifest, "\"release_version\":\"v68\"");
    ...
    assert_contains(manifest, "\"consumer_hint\":\"Node v168 production environment preflight checklist\"");

    assert_contains(manifest, "\"command\":\"cmake -S . -B cmake-build-v68");
    assert_contains(manifest, "\"command\":\"cmake --build cmake-build-v68 --parallel 2\"");
    assert_contains(manifest, "\"command\":\"ctest --test-dir cmake-build-v68 --output-on-failure\"");
    ...
    assert_contains(manifest, "\"CHECKJSON LOAD data/digest-matrix.snap\"");
    assert_contains(manifest, "\"CHECKJSON COMPACT\"");
    assert_contains(manifest, "\"CHECKJSON SETNXEX digest:token 30 value\"");
    assert_contains(manifest, "\"STORAGEJSON\"");
    assert_contains(manifest, "\"HEALTH\"");
    assert_contains(manifest, "\"GET digest:token\"");
    ...
    assert_contains(manifest, "\"cmake_project_version\":\"0.68.0\"");
    ...
    assert_contains(cmake_lists, "project(mini_kv VERSION 0.68.0");
    ...
    assert_contains(result.response, "\"version\":\"0.68.0\"");

...
int main() {
    ...
    assert_contains(smoke, "\"credential_resolver_runtime_shell_chain_stop_or_prerequisite_non_participation_receipt\"");
    assert_contains(
        smoke,
    ...
    assert_contains(
        smoke,
        "SMOKEJSON v134 stop/prerequisite receipt keeps prerequisite_count=6, missing_runtime_prerequisite_count=6, no_go_condition_count=8, check_count=17, and passed_check_count=17");
    assert_contains(smoke,
                    "SMOKEJSON v132 decision record receipt keeps Node v299 decisionDigest=4f6f73fa2806a9ba74174d7bbab17b43459bd1d790237276d95a3937c646e9c0 and runtimeShellDecision=blocked");
    assert_contains(smoke,
    ...
    assert_contains(result.response, "\"runtime_shell_post_decision_plan_intake_non_participation_receipt_only\":true");
    assert_contains(result.response, "\"receipt_digest\":\"fnv1a64:d2739fd08da1653e\"");
    assert_contains(result.response, "\"runtime_shell_chain_stop_or_prerequisite_non_participation_receipt_only\":true");
    assert_contains(result.response, "\"receipt_digest\":\"fnv1a64:7178c69528e0c208\"");

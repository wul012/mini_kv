...
int main() {
    ...
    assert(stages::first_signed_approval_capture_artifact_draft_text_package_review_closeout_audit_release_number() ==
           811);
    assert(stages::planned_signed_approval_capture_artifact_draft_text_package_review_closeout_audit_stage_count() ==
           25);
    ...
    assert(records.front().stage == std::string{"route-preview-value-supply-signed-approval-capture-artifact-draft-"
                                                "text-package-review-closeout-audit-source-freeze"});

...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageComparedPackageEvidenceIntakeAuditStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-compared-package-evidence-intake-identity-binding-slot\"");
    ...
    assert_contains(json, "\"completedIntakeGuardCount\":3");
    ...
    assert(digest.find("v867-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-compared-package-evidence-intake-identity-binding-slot") != std::string::npos);
    assert(audit::published_stage_count() == 7);
}

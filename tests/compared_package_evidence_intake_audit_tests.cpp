...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageComparedPackageEvidenceIntakeAuditStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-compared-package-evidence-intake-manual-submission-reference-slot\"");
    ...
    assert(digest.find("v863-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-compared-package-evidence-intake-manual-submission-reference-slot") != std::string::npos);
    assert(audit::published_stage_count() == 3);
}

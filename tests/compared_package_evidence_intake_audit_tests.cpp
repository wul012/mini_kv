...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageComparedPackageEvidenceIntakeAuditStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-compared-package-evidence-intake-archive-closeout-slot\"");
    ...
    assert_contains(json, "\"completedEvidenceSlotCount\":10");
    ...
    assert(digest.find("v879-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-compared-package-evidence-intake-archive-closeout-slot") != std::string::npos);

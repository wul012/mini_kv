...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v390\"");
    ...
    assert_contains(json, "\"verificationStage\":\"route-preview-verification-report-archive-closeout-verification-retention-map\"");
    assert_contains(json, "\"verificationStageSequence\":10");
    assert_contains(json, "\"verificationReleaseVersion\":\"v390\"");
    assert_contains(json, "\"publishedStageCount\":10");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v389\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v389.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v390 as read-only shard route preview verification report archive closeout verification retention-map evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v389\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v389.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v390 as read-only shard route preview verification report archive closeout verification retention-map evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"verificationStage\":\"route-preview-verification-report-archive-closeout-verification-retention-map\"");
    assert_contains(result.response, "\"verificationReleaseVersion\":\"v390\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v389\"");

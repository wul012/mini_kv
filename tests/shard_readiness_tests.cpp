...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v384\"");
    ...
    assert_contains(json, "\"verificationStage\":\"route-preview-verification-report-archive-closeout-verification-checkjson-contract\"");
    assert_contains(json, "\"verificationStageSequence\":4");
    assert_contains(json, "\"verificationReleaseVersion\":\"v384\"");
    assert_contains(json, "\"publishedStageCount\":4");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v383\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v383.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v384 as read-only shard route preview verification report archive closeout verification checkjson-contract evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v383\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v383.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v384 as read-only shard route preview verification report archive closeout verification checkjson-contract evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"verificationStage\":\"route-preview-verification-report-archive-closeout-verification-checkjson-contract\"");
    assert_contains(result.response, "\"verificationReleaseVersion\":\"v384\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v383\"");

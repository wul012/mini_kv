...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v270 as maintenance-only boundary catalog index formatter split evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v269\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v269.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v269 as maintenance-only boundary catalog group table split evidence only\"");
    ...
    assert_contains(json, "\"sourceFrozenDigest\":\"fnv1a64:d18e6a49fe86a53d\"");

...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v410\"");
    ...
    assert_contains(json, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-retention-map\"");
    assert_contains(json, "\"auditStageSequence\":10");
    assert_contains(json, "\"auditReleaseVersion\":\"v410\"");
    assert_contains(json, "\"publishedStageCount\":10");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v409\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v409.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v410 as read-only shard route preview verification report archive closeout verification audit retention-map evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v409\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v409.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v410 as read-only shard route preview verification report archive closeout verification audit retention-map evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-retention-map\"");
    assert_contains(result.response, "\"auditReleaseVersion\":\"v410\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v409\"");

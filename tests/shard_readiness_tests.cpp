...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v405\"");
    ...
    assert_contains(json, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-cli-smoke-contract\"");
    assert_contains(json, "\"auditStageSequence\":5");
    assert_contains(json, "\"auditReleaseVersion\":\"v405\"");
    assert_contains(json, "\"publishedStageCount\":5");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v404\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v404.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v405 as read-only shard route preview verification report archive closeout verification audit cli-smoke-contract evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v404\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v404.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v405 as read-only shard route preview verification report archive closeout verification audit cli-smoke-contract evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-cli-smoke-contract\"");
    assert_contains(result.response, "\"auditReleaseVersion\":\"v405\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v404\"");

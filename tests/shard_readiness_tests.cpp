...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v356\"");
    ...
    assert_contains(json, "\"archiveStage\":\"route-preview-verification-report-archive-no-router-audit\"");
    assert_contains(json, "\"archiveStageSequence\":16");
    assert_contains(json, "\"archiveReleaseVersion\":\"v356\"");
    assert_contains(json, "\"publishedStageCount\":16");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v355\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v355.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v356 as read-only shard route preview verification report archive no router audit evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v355\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v355.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v356 as read-only shard route preview verification report archive no router audit evidence only\"");
    ...
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v355\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v355.json\"");
    assert_contains(json, "\"sourceFrozenDigest\":\"fnv1a64:34814735ed068cec\"");
    assert_contains(json, "\"continuityStage\":\"feature-read-only-shard-route-preview-verification-report-archive-no-router-audit\"");
    assert_contains(json, "\"stageSequence\":156");
    assert_contains(json, "\"stageReleaseVersion\":\"v356\"");
    ...
    assert_contains(json, "\"trackedPostCloseoutRangeEnd\":\"v356\"");
    assert_contains(json, "\"trackedPostCloseoutReleaseCount\":156");
...
int main() {
    ...
    assert_contains(result.response, "\"archiveStage\":\"route-preview-verification-report-archive-no-router-audit\"");
    assert_contains(result.response, "\"executionAllowed\":false");
}

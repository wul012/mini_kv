...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v394\"");
    ...
    assert_contains(json, "\"verificationStage\":\"route-preview-verification-report-archive-closeout-verification-java-echo-handoff\"");
    assert_contains(json, "\"verificationStageSequence\":14");
    assert_contains(json, "\"verificationReleaseVersion\":\"v394\"");
    assert_contains(json, "\"publishedStageCount\":14");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v393\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v393.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v394 as read-only shard route preview verification report archive closeout verification java-echo-handoff evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v393\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v393.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v394 as read-only shard route preview verification report archive closeout verification java-echo-handoff evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"verificationStage\":\"route-preview-verification-report-archive-closeout-verification-java-echo-handoff\"");
    assert_contains(result.response, "\"verificationReleaseVersion\":\"v394\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v393\"");

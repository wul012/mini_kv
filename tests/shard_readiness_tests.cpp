...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v415\"");
    ...
    assert_contains(json, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-java-echo-audit\"");
    assert_contains(json, "\"auditStageSequence\":15");
    assert_contains(json, "\"auditReleaseVersion\":\"v415\"");
    assert_contains(json, "\"publishedStageCount\":15");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v414\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v414.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v415 as read-only shard route preview verification report archive closeout verification audit java-echo-audit evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v414\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v414.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v415 as read-only shard route preview verification report archive closeout verification audit java-echo-audit evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-java-echo-audit\"");
    assert_contains(result.response, "\"auditReleaseVersion\":\"v415\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v414\"");

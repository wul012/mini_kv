...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"archiveStage\":\"route-preview-verification-report-archive-no-write-audit\"");
    assert_contains(json, "\"archiveStageSequence\":17");
    assert_contains(json, "\"archiveReleaseVersion\":\"v357\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v357 as read-only shard route preview verification report archive no write audit evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v356\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v356.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v357 as read-only shard route preview verification report archive no write audit evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"archiveStage\":\"route-preview-verification-report-archive-no-write-audit\"");
    assert_contains(result.response, "\"executionAllowed\":false");
}

...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v404\"");
    ...
    assert_contains(json, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-checkjson-contract\"");
    assert_contains(json, "\"auditStageSequence\":4");
    assert_contains(json, "\"auditReleaseVersion\":\"v404\"");
    assert_contains(json, "\"publishedStageCount\":4");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v403\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v403.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v404 as read-only shard route preview verification report archive closeout verification audit checkjson-contract evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v403\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v403.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v404 as read-only shard route preview verification report archive closeout verification audit checkjson-contract evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-checkjson-contract\"");
    assert_contains(result.response, "\"auditReleaseVersion\":\"v404\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v403\"");

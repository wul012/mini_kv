...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v411\"");
    ...
    assert_contains(json, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-retention-audit\"");
    assert_contains(json, "\"auditStageSequence\":11");
    assert_contains(json, "\"auditReleaseVersion\":\"v411\"");
    assert_contains(json, "\"publishedStageCount\":11");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v410\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v410.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v411 as read-only shard route preview verification report archive closeout verification audit retention-audit evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v410\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v410.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v411 as read-only shard route preview verification report archive closeout verification audit retention-audit evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-retention-audit\"");
    assert_contains(result.response, "\"auditReleaseVersion\":\"v411\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v410\"");

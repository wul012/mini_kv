...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"maintenanceStage\":\"route-preview-archive-maintenance-walkthrough-index\"");
    assert_contains(json, "\"maintenanceStageSequence\":16");
    assert_contains(json, "\"maintenanceReleaseVersion\":\"v476\"");
    assert_contains(json, "\"sharedStageCatalogFormatterApplied\":true");

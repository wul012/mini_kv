...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v402\"");
    ...
    assert_contains(json, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-catalog-alignment\"");
    assert_contains(json, "\"auditStageSequence\":2");
    assert_contains(json, "\"auditReleaseVersion\":\"v402\"");
    assert_contains(json, "\"publishedStageCount\":2");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v401\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v401.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v402 as read-only shard route preview verification report archive closeout verification audit catalog-alignment evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v401\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v401.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v402 as read-only shard route preview verification report archive closeout verification audit catalog-alignment evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-catalog-alignment\"");
    assert_contains(result.response, "\"auditReleaseVersion\":\"v402\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v401\"");

...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"maintenanceStage\":\"route-preview-archive-maintenance-no-write-audit\"");
    assert_contains(json, "\"maintenanceStageSequence\":23");
    assert_contains(json, "\"maintenanceReleaseVersion\":\"v483\"");
    assert_contains(json, "\"sharedStageCatalogFormatterApplied\":true");

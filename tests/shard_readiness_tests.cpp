...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"maintenanceStage\":\"route-preview-archive-maintenance-command-catalog-contract\"");
    assert_contains(json, "\"maintenanceStageSequence\":6");
    assert_contains(json, "\"maintenanceReleaseVersion\":\"v466\"");
    assert_contains(json, "\"sharedStageCatalogFormatterApplied\":true");

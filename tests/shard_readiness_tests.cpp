...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v414\"");
    ...
    assert_contains(json, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-java-echo-handoff\"");
    assert_contains(json, "\"auditStageSequence\":14");
    assert_contains(json, "\"auditReleaseVersion\":\"v414\"");
    assert_contains(json, "\"publishedStageCount\":14");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v413\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v413.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v414 as read-only shard route preview verification report archive closeout verification audit java-echo-handoff evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v413\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v413.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v414 as read-only shard route preview verification report archive closeout verification audit java-echo-handoff evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-java-echo-handoff\"");
    assert_contains(result.response, "\"auditReleaseVersion\":\"v414\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v413\"");

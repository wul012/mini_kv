...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v396\"");
    ...
    assert_contains(json, "\"verificationStage\":\"route-preview-verification-report-archive-closeout-verification-boundary-field-audit\"");
    assert_contains(json, "\"verificationStageSequence\":16");
    assert_contains(json, "\"verificationReleaseVersion\":\"v396\"");
    assert_contains(json, "\"publishedStageCount\":16");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v395\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v395.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v396 as read-only shard route preview verification report archive closeout verification boundary-field-audit evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v395\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v395.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v396 as read-only shard route preview verification report archive closeout verification boundary-field-audit evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"verificationStage\":\"route-preview-verification-report-archive-closeout-verification-boundary-field-audit\"");
    assert_contains(result.response, "\"verificationReleaseVersion\":\"v396\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v395\"");

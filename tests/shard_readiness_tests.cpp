...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v412\"");
    ...
    assert_contains(json, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-consumer-handoff\"");
    assert_contains(json, "\"auditStageSequence\":12");
    assert_contains(json, "\"auditReleaseVersion\":\"v412\"");
    assert_contains(json, "\"publishedStageCount\":12");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v411\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v411.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v412 as read-only shard route preview verification report archive closeout verification audit consumer-handoff evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v411\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v411.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v412 as read-only shard route preview verification report archive closeout verification audit consumer-handoff evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-consumer-handoff\"");
    assert_contains(result.response, "\"auditReleaseVersion\":\"v412\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v411\"");

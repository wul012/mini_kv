...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v406\"");
    ...
    assert_contains(json, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-tcp-smoke-contract\"");
    assert_contains(json, "\"auditStageSequence\":6");
    assert_contains(json, "\"auditReleaseVersion\":\"v406\"");
    assert_contains(json, "\"publishedStageCount\":6");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v405\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v405.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v406 as read-only shard route preview verification report archive closeout verification audit tcp-smoke-contract evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v405\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v405.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v406 as read-only shard route preview verification report archive closeout verification audit tcp-smoke-contract evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-tcp-smoke-contract\"");
    assert_contains(result.response, "\"auditReleaseVersion\":\"v406\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v405\"");

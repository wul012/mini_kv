...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v269 as maintenance-only boundary catalog group table split evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v268\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v268.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v268 as maintenance-only boundary field name table split evidence only\"");
    ...
    assert_contains(json, "\"sourceFrozenDigest\":\"fnv1a64:efe3aa43bc5101cc\"");

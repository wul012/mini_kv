...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v403\"");
    ...
    assert_contains(json, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-explain-contract\"");
    assert_contains(json, "\"auditStageSequence\":3");
    assert_contains(json, "\"auditReleaseVersion\":\"v403\"");
    assert_contains(json, "\"publishedStageCount\":3");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v402\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v402.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v403 as read-only shard route preview verification report archive closeout verification audit explain-contract evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v402\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v402.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v403 as read-only shard route preview verification report archive closeout verification audit explain-contract evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-explain-contract\"");
    assert_contains(result.response, "\"auditReleaseVersion\":\"v403\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v402\"");

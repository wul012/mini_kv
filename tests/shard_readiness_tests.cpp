...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v416\"");
    ...
    assert_contains(json, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-boundary-field-audit\"");
    assert_contains(json, "\"auditStageSequence\":16");
    assert_contains(json, "\"auditReleaseVersion\":\"v416\"");
    assert_contains(json, "\"publishedStageCount\":16");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v415\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v415.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v416 as read-only shard route preview verification report archive closeout verification audit boundary-field-audit evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v415\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v415.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v416 as read-only shard route preview verification report archive closeout verification audit boundary-field-audit evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"auditStage\":\"route-preview-verification-report-archive-closeout-verification-audit-boundary-field-audit\"");
    assert_contains(result.response, "\"auditReleaseVersion\":\"v416\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v415\"");

...
void assert_shard_readiness_contract(const std::string& json) {
    ...
    assert_contains(json, "\"version\":\"" + std::string{minikv::version} + "\",\"releaseVersion\":\"v397\"");
    ...
    assert_contains(json, "\"verificationStage\":\"route-preview-verification-report-archive-closeout-verification-no-router-audit\"");
    assert_contains(json, "\"verificationStageSequence\":17");
    assert_contains(json, "\"verificationReleaseVersion\":\"v397\"");
    assert_contains(json, "\"publishedStageCount\":17");
    assert_contains(json, "\"plannedStageCount\":20");
    assert_contains(json, "\"sourceFrozenReleaseVersion\":\"v396\"");
    assert_contains(json, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v396.json\"");
    ...
    assert_contains(
        json,
        "\"nodeConsumer\":\"Node v550+ may consume v397 as read-only shard route preview verification report archive closeout verification no-router-audit evidence only\"");
    ...
    assert_contains(json, "\"historicalFallback\":{\"previousConsumedReleaseVersion\":\"v396\"");
    assert_contains(json, "\"previousConsumedFixturePath\":\"fixtures/release/shard-readiness-v396.json\"");
    assert_contains(
        json,
        "\"previousConsumptionNodeVersion\":\"Node v550+ may consume v397 as read-only shard route preview verification report archive closeout verification no-router-audit evidence only\"");
...
int main() {
    ...
    assert_contains(result.response, "\"verificationStage\":\"route-preview-verification-report-archive-closeout-verification-no-router-audit\"");
    assert_contains(result.response, "\"verificationReleaseVersion\":\"v397\"");
    assert_contains(result.response, "\"sourceFrozenReleaseVersion\":\"v396\"");

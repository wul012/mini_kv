...
int main() {
    ...
    assert(records.front().profile_code ==
           std::string{"draft_text_package_review_preflight_package_manifest_criterion_profile"});
    assert(records.back().profile_code == std::string{"draft_text_package_review_preflight_closeout_profile"});
    assert(profiles::planned_signed_approval_capture_artifact_draft_text_package_review_preflight_profile_count() ==
           25);

    const auto first =
        profiles::format_signed_approval_capture_artifact_draft_text_package_review_preflight_profiles_json(1);
    assert_contains(first,
                    "\"profileCode\":\"draft_text_package_review_preflight_package_manifest_criterion_profile\"");
    assert_contains(first, "\"draftTextPackageAcceptanceAllowed\":false");
    assert_contains(first, "\"executionAllowed\":false");
    assert(
        first.find(
            "\"profileCode\":\"draft_text_package_review_preflight_instruction_preflight_digest_criterion_profile\"") ==
        std::string::npos);

    const auto all =
        profiles::format_signed_approval_capture_artifact_draft_text_package_review_preflight_profiles_json(99);

...
int main() {
    ...
    assert_contains(archive, "\"archiveStage\":\"route-preview-verification-report-archive-continuity-audit\"");
    assert_contains(archive, "\"archiveStageSequence\":8");
    assert_contains(archive, "\"archiveReleaseVersion\":\"v348\"");
    assert_contains(archive, "\"publishedStageCount\":8");
    assert_contains(archive, "\"plannedStageCount\":20");
    assert_contains(archive, "\"sourceFrozenReleaseVersion\":\"v347\"");
    assert_contains(archive, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v347.json\"");

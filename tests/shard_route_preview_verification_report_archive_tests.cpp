...
int main() {
    ...
    assert_contains(archive, "\"archiveStage\":\"route-preview-verification-report-archive-retention-map\"");
    assert_contains(archive, "\"archiveStageSequence\":5");
    assert_contains(archive, "\"archiveReleaseVersion\":\"v345\"");
    assert_contains(archive, "\"publishedStageCount\":5");
    assert_contains(archive, "\"plannedStageCount\":20");
    assert_contains(archive, "\"sourceFrozenReleaseVersion\":\"v344\"");
    assert_contains(archive, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v344.json\"");

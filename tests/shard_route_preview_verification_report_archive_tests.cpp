...
int main() {
    ...
    assert_contains(archive, "\"archiveStage\":\"route-preview-verification-report-archive-consumer-handoff-audit\"");
    assert_contains(archive, "\"archiveStageSequence\":10");
    assert_contains(archive, "\"archiveReleaseVersion\":\"v350\"");
    assert_contains(archive, "\"publishedStageCount\":10");
    assert_contains(archive, "\"plannedStageCount\":20");
    assert_contains(archive, "\"sourceFrozenReleaseVersion\":\"v349\"");
    assert_contains(archive, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v349.json\"");

...
int main() {
    ...
    assert_contains(archive, "\"archiveStage\":\"route-preview-verification-report-archive-retention-audit\"");
    assert_contains(archive, "\"archiveStageSequence\":6");
    assert_contains(archive, "\"archiveReleaseVersion\":\"v346\"");
    assert_contains(archive, "\"publishedStageCount\":6");
    assert_contains(archive, "\"plannedStageCount\":20");
    assert_contains(archive, "\"sourceFrozenReleaseVersion\":\"v345\"");
    assert_contains(archive, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v345.json\"");

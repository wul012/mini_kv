...
int main() {
    ...
    assert_contains(archive, "\"archiveStage\":\"route-preview-verification-report-archive-java-echo-handoff\"");
    assert_contains(archive, "\"archiveStageSequence\":11");
    assert_contains(archive, "\"archiveReleaseVersion\":\"v351\"");
    assert_contains(archive, "\"publishedStageCount\":11");
    assert_contains(archive, "\"plannedStageCount\":20");
    assert_contains(archive, "\"sourceFrozenReleaseVersion\":\"v350\"");
    assert_contains(archive, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v350.json\"");

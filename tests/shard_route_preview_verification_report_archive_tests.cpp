...
int main() {
    ...
    assert_contains(archive, "\"archiveStage\":\"route-preview-verification-report-archive-no-execution-audit\"");
    assert_contains(archive, "\"archiveStageSequence\":18");
    assert_contains(archive, "\"archiveReleaseVersion\":\"v358\"");
    assert_contains(archive, "\"publishedStageCount\":18");
    assert_contains(archive, "\"plannedStageCount\":20");
    assert_contains(archive, "\"sourceFrozenReleaseVersion\":\"v357\"");
    assert_contains(archive, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v357.json\"");

...
int main() {
    ...
    assert_contains(archive, "\"archiveStage\":\"route-preview-verification-report-archive-consumer-handoff\"");
    assert_contains(archive, "\"archiveStageSequence\":9");
    assert_contains(archive, "\"archiveReleaseVersion\":\"v349\"");
    assert_contains(archive, "\"publishedStageCount\":9");
    assert_contains(archive, "\"plannedStageCount\":20");
    assert_contains(archive, "\"sourceFrozenReleaseVersion\":\"v348\"");
    assert_contains(archive, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v348.json\"");

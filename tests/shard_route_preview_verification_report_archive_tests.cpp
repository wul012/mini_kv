...
int main() {
    ...
    assert_contains(archive, "\"archiveStage\":\"route-preview-verification-report-archive-no-router-audit\"");
    assert_contains(archive, "\"archiveStageSequence\":16");
    assert_contains(archive, "\"archiveReleaseVersion\":\"v356\"");
    assert_contains(archive, "\"publishedStageCount\":16");
    assert_contains(archive, "\"plannedStageCount\":20");
    assert_contains(archive, "\"sourceFrozenReleaseVersion\":\"v355\"");
    assert_contains(archive, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v355.json\"");

...
int main() {
    ...
    assert_contains(archive, "\"archiveStage\":\"route-preview-verification-report-archive-tcp-smoke-contract\"");
    assert_contains(archive, "\"archiveStageSequence\":15");
    assert_contains(archive, "\"archiveReleaseVersion\":\"v355\"");
    assert_contains(archive, "\"publishedStageCount\":15");
    assert_contains(archive, "\"plannedStageCount\":20");
    assert_contains(archive, "\"sourceFrozenReleaseVersion\":\"v354\"");
    assert_contains(archive, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v354.json\"");

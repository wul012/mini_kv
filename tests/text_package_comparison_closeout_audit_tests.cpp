...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageComparisonCloseoutAuditStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-comparison-closeout-audit-comparison-lane-catalog-sequence\"");
    ...
    assert_contains(json, "\"completedComparisonCheckCount\":3");
    ...
    assert(digest.find("v838-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-comparison-closeout-audit-comparison-lane-catalog-sequence") != std::string::npos);

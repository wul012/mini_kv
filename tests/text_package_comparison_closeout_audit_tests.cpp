...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageComparisonCloseoutAuditStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-comparison-closeout-audit-source-evidence-handle-lane\"");
    ...
    assert_contains(json, "\"completedComparisonCheckCount\":12");
    ...
    assert(digest.find("v847-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-comparison-closeout-audit-source-evidence-handle-lane") != std::string::npos);

...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageComparisonCloseoutAuditStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-comparison-closeout-audit-execution-lock-control\"");
    ...
    assert_contains(json, "\"completedComparisonCheckCount\":21");
    ...
    assert(digest.find("v856-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-comparison-closeout-audit-execution-lock-control") != std::string::npos);
    assert(audit::published_stage_count() == 21);
}

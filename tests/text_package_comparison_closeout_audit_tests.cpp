...
int main() {
    ...
    assert_contains(json, "\"draftTextPackageComparisonCloseoutAuditStage\":\"route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-comparison-closeout-audit-uncompared-package-rejection\"");
    ...
    assert_contains(json, "\"completedComparisonCheckCount\":18");
    ...
    assert(digest.find("v853-route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-comparison-closeout-audit-uncompared-package-rejection") != std::string::npos);

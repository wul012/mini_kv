...
int main() {
    ...
    assert(stages::first_signed_approval_capture_artifact_draft_instruction_preflight_release_number() == 761);
    assert(stages::planned_signed_approval_capture_artifact_draft_instruction_preflight_stage_count() == 25);
    assert(records.front().sequence == 1);
    assert(records.front().release_version == std::string{"v761"});
    assert(records.front().source_frozen_release_version == std::string{"v760"});
    assert(records.back().sequence == 25);
    assert(records.back().release_version == std::string{"v785"});

...
void verify_release_inputs() {
    ...
    assert_contains(manifest, "\"SMOKEJSON exposes live_read_session.session_id_echo=mini-kv-live-read-v102 and "
                              "read_command_list_digest=fnv1a64:5bef33f2fbe65cc5\"");
    assert_contains(manifest, "\"SMOKEJSON exposes binary_provenance.provenance_digest=fnv1a64:d9776044d393ecbc and "
                              "production_binary_claimed=false\"");
    assert_contains(manifest, "\"SMOKEJSON exposes retention_provenance_check.check_digest=fnv1a64:8cf7cc0df218035f "
                              "and managed_audit_write_executed=false\"");
    assert_contains(manifest,
                    "\"SMOKEJSON exposes retention_provenance_replay_marker.marker_digest=fnv1a64:6b9ef93f40cbd343 and "
                    "replay_executed=false\"");
    assert_contains(
        manifest,
        "\"SMOKEJSON exposes managed_audit_adapter_restore_boundary_receipt.receipt_digest=fnv1a64:1177efbaceb52c1c "
        "and adapter_write_allowed=false\"");
    assert_contains(manifest,
                    "\"SMOKEJSON exposes adapter_shell_non_storage_guard_receipt.consumed_release_version=v88\"");
    assert_contains(
        manifest,
        "\"SMOKEJSON exposes adapter_shell_non_storage_guard_receipt.receipt_digest=fnv1a64:0174fa831d17cea4\"");
    assert_contains(
        manifest,
        "\"SMOKEJSON exposes managed_audit_external_adapter_non_participation_receipt.consumed_release_version=v89\"");
    assert_contains(
        manifest, "\"SMOKEJSON exposes "
                  "managed_audit_external_adapter_non_participation_receipt.receipt_digest=fnv1a64:9bacde73d6d07097\"");
    assert_contains(
        manifest, "\"SMOKEJSON exposes "
                  "managed_audit_external_adapter_non_participation_receipt.external_adapter_storage_backend=false\"");
    assert_contains(
        manifest,
        "\"SMOKEJSON exposes managed_audit_external_adapter_non_participation_receipt.credential_read_allowed=false\"");
    assert_contains(
        manifest,
        "\"SMOKEJSON exposes managed_audit_sandbox_adapter_non_participation_receipt.consumed_release_version=v90\"");
    assert_contains(
        manifest, "\"SMOKEJSON exposes "
                  "managed_audit_sandbox_adapter_non_participation_receipt.receipt_digest=fnv1a64:f0cae7a4ce0674c2\"");
    assert_contains(manifest,
                    "\"SMOKEJSON exposes "
                    "managed_audit_sandbox_adapter_non_participation_receipt.sandbox_adapter_storage_backend=false\"");
    assert_contains(manifest,
                    "\"SMOKEJSON exposes "
                    "managed_audit_sandbox_adapter_non_participation_receipt.credential_value_read_allowed=false\"");
    assert_contains(
        manifest,
        "\"SMOKEJSON exposes "
        "managed_audit_sandbox_adapter_non_participation_receipt.sandbox_managed_audit_state_write_allowed=false\"");
    assert_contains(
        manifest,
        "\"SMOKEJSON exposes managed_audit_sandbox_connection_receipt_echo_marker.consumed_release_version=v95\"");
    assert_contains(
        manifest,
        "\"SMOKEJSON exposes managed_audit_sandbox_connection_receipt_echo_marker.consumed_artifact_path_hint=c/95/\"");
    assert_contains(
        manifest,
        "\"SMOKEJSON exposes "
        "managed_audit_sandbox_connection_receipt_echo_marker.consumed_receipt_digest=fnv1a64:ceaed265f7f9560c\"");
    assert_contains(manifest, "\"SMOKEJSON exposes "
                              "managed_audit_sandbox_connection_receipt_echo_marker.source_operator_packet_profile="
                              "managed-audit-manual-sandbox-connection-operator-packet.v1\"");
    assert_contains(manifest, "\"SMOKEJSON exposes "
                              "managed_audit_sandbox_connection_receipt_echo_marker.packet_mode=manual-sandbox-"
                              "connection-operator-packet-only\"");
    assert_contains(manifest,
                    "\"SMOKEJSON exposes "
                    "managed_audit_sandbox_connection_receipt_echo_marker.marker_digest=fnv1a64:beb8dd6a0b102a11\"");
    assert_contains(manifest,
                    "\"SMOKEJSON exposes "
                    "managed_audit_sandbox_connection_receipt_echo_marker.connection_execution_allowed=false\"");
    assert_contains(manifest,
                    "\"SMOKEJSON exposes "
                    "managed_audit_sandbox_connection_receipt_echo_marker.credential_value_read_allowed=false\"");
    assert_contains(
        manifest,
        "\"SMOKEJSON exposes managed_audit_sandbox_connection_receipt_echo_marker.managed_audit_write_allowed=false\"");
    assert_contains(
        manifest,
        "\"SMOKEJSON exposes managed_audit_sandbox_connection_receipt_echo_marker.node_auto_start_allowed=false\"");
    assert_contains(
        manifest,
        "\"SMOKEJSON exposes managed_audit_sandbox_connection_no_start_guard_receipt.consumed_release_version=v96\"");
    assert_contains(
        manifest, "\"SMOKEJSON exposes "
                  "managed_audit_sandbox_connection_no_start_guard_receipt.receipt_digest=fnv1a64:69e377d01f15fd57\"");
    assert_contains(
        manifest,
        "\"SMOKEJSON exposes managed_audit_sandbox_connection_no_start_guard_receipt.node_auto_start_allowed=false\"");
    assert_contains(manifest,
                    "\"SMOKEJSON exposes "
                    "managed_audit_sandbox_connection_no_start_guard_receipt.mini_kv_auto_start_allowed=false\"");
    assert_contains(manifest, "\"managed_audit_adapter_non_authoritative_storage_receipt\":{\"consumer\":\"Node v217 "
    ...
    assert_contains(manifest,
                    "\"SMOKEJSON and INFOJSON expose ci_evidence.artifact_path_hint=c/80/ and no_restore_proof=true\"");
    assert_contains(manifest, "\"SMOKEJSON and INFOJSON expose artifact_retention.retention_days=30 and "
                              "github_artifact_upload_attempted=false\"");
    assert_contains(
        manifest,
        "\"SMOKEJSON exposes failure taxonomy categories and taxonomy_digest for imported window verification\"");
    assert_contains(manifest,
                    "\"SMOKEJSON exposes operator_window.identity_neutral_proof=true for archive handoff checks\"");
    ...
    assert_contains(
        manifest,
        "SMOKEJSON exposes credential_resolver_implementation_candidate_gate_input_hardening_non_participation_receipt "
        "for Node v330");
    assert_contains(manifest,
                    "SMOKEJSON exposes credential_resolver_abort_rollback_semantics_contract_non_participation_receipt "
                    "for Node v327");
    assert_contains(manifest,
                    "SMOKEJSON exposes "
                    "credential_resolver_no_network_safety_fixture_contract_non_participation_receipt for Node v324");
    ...
    assert_contains(manifest,
                    "SMOKEJSON exposes credential_resolver_disabled_implementation_candidate_non_participation_receipt "
                    "for Node v274");
    ...
    assert_contains(
        ...
        "for Node v282");
    assert_contains(manifest, "\"source_profile_version\":\"managed-audit-manual-sandbox-connection-credential-"
                              "resolver-approval-required-implementation-readiness-review.v1\"");
    assert_contains(
        manifest,
    ...
    assert_contains(manifest, "\"profile_version\":\"managed-audit-manual-sandbox-connection-credential-resolver-no-"
                              "network-safety-fixture-contract-intake.v1\"");
    ...
    assert_contains(manifest, "\"profile_version\":\"managed-audit-manual-sandbox-connection-credential-resolver-abort-"
                              "rollback-semantics-contract-intake.v1\"");
    ...
    assert_contains(manifest, "\"profile_version\":\"managed-audit-manual-sandbox-connection-credential-resolver-"
                              "implementation-candidate-gate-input-hardening-decision.v1\"");
    assert_contains(manifest,
                    "\"candidate_gate_state\":\"implementation-candidate-gate-input-hardening-decision-ready\"");
    assert_contains(manifest,
    ...
    assert_contains(manifest, "\"source_profile_version\":\"managed-audit-manual-sandbox-connection-credential-"
                              "resolver-disabled-candidate-upstream-echo-verification.v1\"");
    assert_contains(
        manifest,
    ...
    assert_contains(manifest, "\"boundary_codes\":[\"CREDENTIAL_HANDLE\",\"ENDPOINT_HANDLE\",\"OPERATOR_APPROVAL\","
    ...
    assert_contains(
        manifest,
        "\"source_archive_verification_state\":\"credential-resolver-fake-shell-archive-verification-ready\"");
    ...
    assert_contains(manifest, "SMOKEJSON exposes "
                              "credential_resolver_disabled_fake_harness_execution_denied_receipt_verification_"
                              "retention_check for Node v293");
    assert_contains(manifest, "SMOKEJSON v129 retention check keeps source Node v292 readinessDecision=blocked");
    ...
    assert_contains(manifest, "SMOKEJSON v130 disabled runtime shell receipt keeps source Node v295 "
    ...
    assert_contains(
        manifest, "SMOKEJSON exposes "
                  "credential_resolver_disabled_runtime_shell_candidate_gate_non_participation_receipt for Node v298");
    ...
    assert_contains(
        manifest,
        "SMOKEJSON exposes credential_resolver_runtime_shell_post_decision_plan_intake_non_participation_receipt for "
        "Node v302");
    assert_contains(manifest,
                    "\"credential_resolver_runtime_shell_chain_stop_or_prerequisite_non_participation_receipt\"");
    assert_contains(
        manifest,
        "SMOKEJSON exposes credential_resolver_runtime_shell_chain_stop_or_prerequisite_non_participation_receipt for "
        "Node v305");
    assert_contains(manifest,
                    "\"credential_resolver_approval_prerequisite_artifact_intake_non_participation_receipt\"");
    assert_contains(
        manifest, "SMOKEJSON exposes "
                  "credential_resolver_approval_prerequisite_artifact_intake_non_participation_receipt for Node v307");
    ...
    assert_contains(manifest,
                    "SMOKEJSON v131 candidate gate receipt keeps Node v297 "
                    "gateDigest=651383bcd175bdaff2691c026135a1cebbcf30de91be7709cbc7843866684e22 and blocked decision");
    ...
    assert_contains(manifest,
                    "\"command_response_formatters_header\":\"include/minikv/command_response_formatters.hpp\"");
    ...
    assert_contains(manifest,
                    "\"node_action\":\"verify retention days and artifact path before cross-project retention gate\"");
    assert_contains(
        manifest,
    ...
    assert_contains(manifest, "\"runtime_binary_hint\":\"cmake-build-v102/minikv_server and "
                              "cmake-build-v102/minikv_client from the current CMake build\"");
    ...
    assert_contains(manifest, "\"verify binary, fixture, and release evidence path alignment before managed audit "
                              "persistence boundary work\"");
    assert_contains(manifest, "\"retention_provenance_check\":{\"consumer\":\"Node v211 managed audit identity "
    ...
    assert_contains(manifest, "\"verify runtime evidence retention and binary provenance alignment before managed "
                              "audit dry-run packet work\"");
    assert_contains(
        manifest,
    ...
    assert_contains(manifest, "\"managed_audit_adapter_restore_boundary_receipt\":{\"consumer\":\"Node v215 managed "
                              "audit dry-run adapter candidate\"");
    ...
    assert_contains(manifest, "\"Node v208 may verify SMOKEJSON and INFOJSON binary provenance hints before managed "
                              "audit persistence boundary work\"");
    assert_contains(manifest, "\"Node v211 may verify runtime evidence retention and provenance check hints before "
                              "managed audit identity approval provenance dry-run packet work\"");
    assert_contains(manifest, "\"Node v213 may verify the v84 retention provenance replay marker before managed audit "
                              "packet restore drill planning\"");
    assert_contains(manifest, "\"node_action\":\"verify mini-kv storage is non-authoritative before managed audit "
                              "adapter production-hardening readiness gate\"");
    assert_contains(manifest, "\"command_dispatch_quality_receipt\":{\"consumer\":\"Node v219 managed audit adapter "
                              "implementation precheck packet\"");
    ...
    assert_contains(manifest, "\"adapter_shell_non_storage_guard_receipt\":{\"consumer\":\"Node v221 managed audit "
    ...
    assert_contains(manifest, "\"managed_audit_external_adapter_non_participation_receipt\":{\"consumer\":\"Node v223 "
    ...
    assert_contains(manifest, "\"managed_audit_write_executed\":false");
    assert_contains(manifest, "\"managed_audit_sandbox_adapter_non_participation_receipt\":{\"consumer\":\"Node v225 "
    ...
    assert_contains(manifest, "\"managed_audit_sandbox_connection_receipt_echo_marker\":{\"consumer\":\"Node v229 "
                              "manual sandbox connection packet verification\"");
    ...
    assert_contains(
        manifest, "\"source_operator_packet_profile\":\"managed-audit-manual-sandbox-connection-operator-packet.v1\"");
    assert_contains(manifest, "\"packet_mode\":\"manual-sandbox-connection-operator-packet-only\"");
    assert_contains(manifest,
                    "\"owner_approval_artifact_id_field\":\"ORDEROPS_MANAGED_AUDIT_OWNER_APPROVAL_ARTIFACT_ID\"");
    ...
    assert_contains(manifest, "\"managed_audit_sandbox_connection_no_start_guard_receipt\":{\"consumer\":\"Node v231 "
                              "manual sandbox connection preflight verification\"");
    ...
    assert_contains(
        manifest, "\"Binary provenance hint is read-only path alignment evidence, not production binary attestation\"");
    assert_contains(manifest,
                    "\"Node v205 may verify SMOKEJSON live-read session hints before the real-read execution packet\"");
    assert_contains(manifest, "\"Live-read session hint is read-only evidence, not permission to auto-start mini-kv\"");
    assert_contains(manifest, "\"Node v203 may verify SMOKEJSON and INFOJSON artifact retention evidence before the "
                              "cross-project retention gate\"");
    assert_contains(manifest, "\"Artifact retention evidence is read-only rehearsal metadata, not a real GitHub "
                              "artifact retention configuration\"");
    assert_contains(manifest, "\"Node v201 may still verify SMOKEJSON and INFOJSON CI evidence hints before checking "
                              "the CI artifact manifest\"");
    assert_contains(manifest, "\"CI evidence hint is read-only path evidence, not a real GitHub artifact upload\"");
    assert_contains(manifest, "\"SMOKEJSON operator-window proof is identity-neutral evidence, not authentication or "
                              "production authorization\"");
    assert_contains(manifest, "\"Node v223 may verify the mini-kv external adapter non-participation receipt before "
                              "external adapter connection readiness review\"");
    assert_contains(manifest, "\"Managed audit external adapter non-participation receipt is read-only evidence that "
                              "mini-kv is not external adapter storage and reads no credentials\"");
    assert_contains(manifest, "\"Node v225 may verify the mini-kv sandbox adapter non-participation receipt before "
                              "sandbox adapter dry-run package\"");
    assert_contains(manifest,
                    "\"Managed audit sandbox adapter non-participation receipt is read-only evidence that mini-kv is "
                    "not sandbox audit storage, reads no credentials, and writes no managed audit state\"");
    assert_contains(manifest, "\"Node v231 may verify the mini-kv sandbox connection no-start guard receipt before "
                              "manual sandbox connection preflight verification\"");
    assert_contains(manifest,
                    "\"Managed audit sandbox connection receipt echo marker echoes field names only; mini-kv does not "
                    "connect, read credential values, run schema rehearsal, or write managed audit state\"");
    assert_contains(manifest, "\"Managed audit receipt formatter module keeps digest/json formatting outside "
                              "command.cpp while preserving read-only runtime contracts\"");
    assert_contains(manifest, "\"Runtime evidence receipt formatter module keeps live-read, taxonomy, binary "
                              "provenance, retention provenance, and operator-window formatting outside command.cpp "
                              "while preserving INFOJSON/SMOKEJSON field contracts\"");
    assert_contains(manifest, "\"Command contract formatter module keeps command catalog, EXPLAINJSON, and CHECKJSON "
                              "formatting outside command.cpp while preserving read-only command contract fields\"");
    assert_contains(manifest,
                    "\"Command response formatter module keeps KEYS, INFO, STATS, HEALTH, SMOKEJSON, STORAGEJSON, and "
                    "WALINFO formatting outside command.cpp while preserving runtime response field contracts\"");
    ...
    assert_contains(test_manifest, "minikv_runtime_artifact_bundle_manifest_tests");
    assert_contains(test_manifest, "minikv_restore_compatibility_handoff_tests");
    assert_contains(test_manifest, "minikv_restore_dry_run_operator_package_tests");
    assert_contains(test_manifest, "minikv_artifact_digest_compatibility_matrix_tests");
    assert_contains(test_manifest, "minikv_release_artifact_digest_package_tests");
    assert_contains(test_manifest, "minikv_restore_drill_evidence_tests");
    assert_contains(test_manifest, "minikv_restore_handoff_checklist_tests");
    assert_contains(test_manifest, "minikv_restore_evidence_retention_tests");
    assert_contains(test_manifest, "minikv_retained_restore_artifact_digest_tests");
    assert_contains(test_manifest, "minikv_restore_approval_boundary_tests");
    assert_contains(test_manifest, "minikv_restore_boundary_smoke_manifest_tests");
    assert_contains(test_manifest, "minikv_runtime_smoke_evidence_tests");
    assert_contains(test_manifest, "minikv_manual_sandbox_dry_run_command_non_participation_receipt_tests");
    assert_contains(test_manifest, "minikv_manual_sandbox_connection_precheck_non_participation_receipt_tests");
    assert_contains(test_manifest, "minikv_sandbox_endpoint_handle_non_participation_receipt_tests");
}

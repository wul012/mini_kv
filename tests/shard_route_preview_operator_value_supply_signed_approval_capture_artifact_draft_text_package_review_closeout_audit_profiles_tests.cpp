...
int main() {
    ...
    assert_contains(first, "\"profileCode\":\"draft_text_package_review_closeout_audit_source_freeze_profile\"");
    assert_contains(first, "\"boundary\":\"source-fixture-only\"");
    assert_contains(first, "\"draftTextPackageAcceptanceAllowed\":false");
    assert(first.find("\"profileCode\":\"draft_text_package_review_closeout_audit_release_range_profile\"") ==
           std::string::npos);

    const auto all =
        profiles::format_signed_approval_capture_artifact_draft_text_package_review_closeout_audit_profiles_json(99);

...
int main() {
    ...
    assert_contains(
        closeout,
        "\"closeoutStage\":\"route-preview-verification-report-archive-closeout-verification-audit-closeout-retention-map\"");
    assert_contains(closeout, "\"closeoutStageSequence\":10");
    assert_contains(closeout, "\"closeoutReleaseVersion\":\"v430\"");
    assert_contains(closeout, "\"publishedStageCount\":10");
    assert_contains(closeout, "\"plannedStageCount\":20");
    assert_contains(closeout, "\"sourceFrozenReleaseVersion\":\"v429\"");
    assert_contains(closeout, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v429.json\"");
    ...
    assert(occurrence_count(shard, "\"shardRoutePreviewVerificationReportArchiveCloseoutVerificationAuditCloseout\":") ==
           1);
    assert_contains(shard, "\"closeoutReleaseVersion\":\"v430\"");

...
int main() {
    ...
    assert_contains(
        closeout,
        "\"closeoutStage\":\"route-preview-verification-report-archive-closeout-verification-audit-closeout-no-write-audit\"");
    assert_contains(closeout, "\"closeoutStageSequence\":18");
    assert_contains(closeout, "\"closeoutReleaseVersion\":\"v438\"");
    assert_contains(closeout, "\"publishedStageCount\":18");
    assert_contains(closeout, "\"plannedStageCount\":20");
    assert_contains(closeout, "\"sourceFrozenReleaseVersion\":\"v437\"");
    assert_contains(closeout, "\"sourceFrozenFixturePath\":\"fixtures/release/shard-readiness-v437.json\"");
    ...
    assert(occurrence_count(shard, "\"shardRoutePreviewVerificationReportArchiveCloseoutVerificationAuditCloseout\":") ==
           1);
    assert_contains(shard, "\"closeoutReleaseVersion\":\"v438\"");

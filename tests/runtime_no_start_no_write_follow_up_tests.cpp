...
void assert_current_digest_set(const std::string& text) {
    assert_contains(text, "\"current_project_version\":\"0.101.0\"");
    assert_contains(text, "\"current_release_version\":\"v101\"");
    assert_contains(text, "\"current_artifact_path_hint\":\"c/101/\"");
    assert_contains(text, "\"current_live_read_session_echo\":\"mini-kv-live-read-v101\"");
...
int main() {
    ...
    assert_contains(follow, "\"consumer_hint\":\"Node v237 manual sandbox connection readiness gate\"");
    ...
    assert_contains(smoke, "\"runtime_smoke_evidence_version\":\"mini-kv-runtime-smoke-evidence.v15\"");

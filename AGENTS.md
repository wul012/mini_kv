...
当上下文明显变长、即将进入压缩风险区时，停止继续执行新的代码修改或版本推进。

执行要求：

- 先向用户说明当前已经完成到哪里、还剩哪些关键步骤、哪些文件或进程需要注意。
- 明确建议用户压缩上下文后再继续。
- 在用户完成压缩或明确要求继续前，不再启动新的实现、重构、测试扩展或提交动作。
- 如果只是补充这条规则、清理当前任务产生的临时文件、停止本轮已启动的后台进程，可以继续完成这些收尾动作。

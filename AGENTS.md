...
- 从下一版开始，新增代码讲解放入符合当前项目进度的阶段文件夹，避免根目录继续堆大量文件。当前阶段文件夹命名为：
...
```text
代码讲解记录/生产就绪证据阶段/113-example-topic-v57.md
```

- 讲解版式、编号延续、`代码讲解记录/README.md` 顶层索引方式保持不变；索引应指向阶段文件夹内的实际文件。
- 如果同一版本有多篇讲解，全部放在同一个阶段文件夹中，并通过文件名区分主题。
- 历史讲解文件不主动搬迁，除非用户明确要求整理旧文件，避免产生大规模无意义改名 diff。

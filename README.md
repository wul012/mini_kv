...
- Runtime `INFOJSON` and `SMOKEJSON` expose v87 `managed_audit_adapter_non_authoritative_storage_receipt` evidence for Node v217: mini-kv is not the managed audit store, does not allow managed audit storage writes, does not allow admin/restore execution, and is not Java order authority

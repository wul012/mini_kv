...
Version 97 is a runnable in-memory KV service with injected build version metadata, command catalog metadata, bounded JSON key inventory, digest-aware structured command risk explanation, read-only write-command execution contracts, a short-TTL token primitive with recovery evidence, release verification, runtime artifact rollback, runtime artifact bundle, restore compatibility handoff, restore dry-run operator package, artifact digest compatibility matrix, release artifact digest package, restore drill evidence, restore handoff checklist, restore evidence retention, retained restore artifact digest, restore approval boundary, restore boundary smoke manifests, structured runtime smoke evidence, SMOKEJSON failure taxonomy digest verification for imported real-read window results, an operator-window identity-neutral proof for archived real-read window checks, read-only CI/evidence hints for Node v201 manifest verification, read-only artifact retention evidence for Node v203 cross-project retention gate checks, live-read session hints for Node v205 real-read execution packet preparation, binary provenance and retention provenance hints for managed audit preparation, a managed audit adapter restore boundary receipt, a managed audit adapter non-authoritative storage receipt for Node v217 readiness gating, a v88 command dispatch quality receipt for Node v219 implementation precheck, a v89 adapter shell non-storage guard receipt for Node v221 local adapter candidate dry-run, a v90 managed audit external adapter non-participation receipt for Node v223 connection readiness review, a v95 managed audit sandbox adapter non-participation receipt for Node v225 sandbox adapter dry-run package, a v96 managed audit sandbox connection receipt echo marker for Node v229 manual sandbox connection packet verification, and a v97 managed audit sandbox connection no-start guard receipt for Node v231 manual sandbox connection preflight verification, plus contract-preserving formatter splits for managed audit receipts (`include/minikv/managed_audit_receipts.hpp`, `src/managed_audit_receipts.cpp`), runtime evidence receipts (`include/minikv/runtime_evidence_receipts.hpp`, `src/runtime_evidence_receipts.cpp`), command contracts (`include/minikv/command_contracts.hpp`, `src/command_contracts.cpp`), and command response formatters (`include/minikv/command_response_formatters.hpp`, `src/command_response_formatters.cpp`), stable CHECKJSON write/read fixture samples, self-describing INFOJSON/STATSJSON/SMOKEJSON read-only runtime evidence, an INFOJSON/STATSJSON field guide for release evidence review, read-only storage evidence, and indexed restart recovery evidence with restart/replay cost and retention boundaries:
...
- Repository fixture `fixtures/release/runtime-smoke-evidence.json` records the v97 SMOKEJSON real-read command target for Node v231 manual sandbox connection preflight verification: it uses `SMOKEJSON`, `INFOJSON`, `STORAGEJSON`, `HEALTH`, and `GET restore:real-read-token` as read-only runtime evidence, adds live-read session id echo `mini-kv-live-read-v97`, binary provenance digest `fnv1a64:bc97d5dba7bf5130`, retention provenance digest `fnv1a64:dfb8f391498358f3`, replay marker digest `fnv1a64:70a34f981a518759`, managed audit adapter restore/non-authoritative receipts, the v88 command dispatch receipt, the v89 adapter shell non-storage guard receipt, the v90 external adapter non-participation receipt, the v95 sandbox adapter non-participation receipt `fnv1a64:d71b79804d9cfd94`, the v96 sandbox connection receipt echo marker `fnv1a64:fbad91e89da49997`, and the v97 sandbox connection no-start guard receipt `fnv1a64:9ff902c1a090c0b7` without executing LOAD, COMPACT, SETNXEX, restore, CI upload, production window opening, managed audit writes, adapter storage writes, credential value reads, external or sandbox adapter migration, schema rehearsal, sandbox connection execution, Node/Java/mini-kv auto-start, sandbox dry-run record writes, or sandbox managed audit state writes
- Repository fixtures `fixtures/release/runtime-smoke-evidence.json` and `fixtures/release/verification-manifest.json` keep the v90 external adapter non-participation receipt, the v95 sandbox adapter non-participation receipt, the v96 sandbox connection receipt echo marker, and the v97 sandbox connection no-start guard receipt with `connection_execution_allowed=false`, `credential_value_read_allowed=false`, `schema_rehearsal_execution_allowed=false`, `schema_migration_execution_allowed=false`, `managed_audit_write_allowed=false`, `sandbox_adapter_storage_backend=false`, `participates_in_sandbox_connection=false`, `sandbox_managed_audit_state_write_allowed=false`, `node_auto_start_allowed=false`, `java_auto_start_allowed=false`, `mini_kv_auto_start_allowed=false`, `restore_execution_allowed=false`, `load_restore_compact_executed=false`, and `order_authoritative=false`

...
- Repository fixture `fixtures/release/runtime-smoke-evidence.json` records the v91 SMOKEJSON real-read command target for Node v225 managed audit sandbox adapter dry-run package: it uses `SMOKEJSON`, `INFOJSON`, `STORAGEJSON`, `HEALTH`, and `GET restore:real-read-token` as read-only runtime evidence, adds live-read session id echo `mini-kv-live-read-v91`, binary provenance digest `fnv1a64:fcb45d5de96a5cc4`, retention provenance digest `fnv1a64:67d9e038b4f75ddd`, replay marker digest `fnv1a64:ee7d1884932cf285`, managed audit adapter restore/non-authoritative receipts, the v88 command dispatch receipt, the v89 adapter shell non-storage guard receipt, the v90 external adapter non-participation receipt, and the v91 sandbox adapter non-participation receipt `fnv1a64:fd925623ea87eac4` without executing LOAD, COMPACT, SETNXEX, restore, CI upload, production window opening, managed audit writes, adapter storage writes, credential reads, external or sandbox adapter migration, sandbox dry-run record writes, or sandbox managed audit state writes
- Repository fixtures `fixtures/release/runtime-smoke-evidence.json` and `fixtures/release/verification-manifest.json` keep the v90 external adapter non-participation receipt and add the v91 sandbox adapter non-participation receipt with `sandbox_adapter_storage_backend=false`, `participates_in_sandbox_adapter=false`, `credential_value_read_allowed=false`, `production_credential_read_allowed=false`, `schema_migration_execution_allowed=false`, `sandbox_schema_authoritative=false`, `sandbox_managed_audit_state_write_allowed=false`, `sandbox_dry_run_records_written=false`, `managed_audit_write_executed=false`, `restore_execution_allowed=false`, `load_restore_compact_executed=false`, and `order_authoritative=false`

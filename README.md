...
- Runtime `INFOJSON` and `SMOKEJSON` expose the historical `managed_audit_adapter_non_authoritative_storage_receipt` evidence for Node v217, the v88 `command_dispatch_quality_receipt` for Node v219, the v89 `adapter_shell_non_storage_guard_receipt` for Node v221, and the v90 `managed_audit_external_adapter_non_participation_receipt` for Node v223: mini-kv remains read-only runtime evidence, is not the managed audit store, is not the adapter shell or external adapter storage backend, does not read managed audit credentials, does not execute external adapter migrations, does not write local dry-run records, does not allow admin/restore execution, does not change write/admin/WAL/snapshot/restore handlers, and is not Java order authority
...
- Repository fixture `fixtures/release/runtime-smoke-evidence.json` records the v90 SMOKEJSON real-read command target for Node v223 managed audit external adapter connection readiness review: it uses `SMOKEJSON`, `INFOJSON`, `STORAGEJSON`, `HEALTH`, and `GET restore:real-read-token` as read-only runtime evidence, adds live-read session id echo `mini-kv-live-read-v90`, binary provenance digest `fnv1a64:05de09a5ea28b666`, retention provenance digest `fnv1a64:9e97e1284756f0e3`, replay marker digest `fnv1a64:5b19ed3789b3f799`, managed audit adapter restore/non-authoritative receipts, the v88 command dispatch receipt, the v89 adapter shell non-storage guard receipt, and the v90 external adapter non-participation receipt without executing LOAD, COMPACT, SETNXEX, restore, CI upload, production window opening, managed audit writes, adapter storage writes, credential reads, external adapter migration, or local dry-run record writes
- Repository fixtures `fixtures/release/runtime-smoke-evidence.json` and `fixtures/release/verification-manifest.json` now carry the v90 external adapter non-participation receipt with `external_adapter_storage_backend=false`, `participates_in_external_adapter=false`, `credential_read_allowed=false`, `migration_execution_allowed=false`, `schema_authoritative=false`, `managed_audit_write_executed=false`, `restore_execution_allowed=false`, `load_restore_compact_executed=false`, and `order_authoritative=false`
...
`fixtures/release/verification-manifest.json` is the v90 release verification manifest for Node v223 managed audit external adapter connection readiness review: it lists expected CMake configure/build, CTest, targeted tests, read-only `SMOKEJSON` live-read session hints (`session_id_echo="mini-kv-live-read-v90"`, `read_command_list_digest="fnv1a64:5bef33f2fbe65cc5"`, `write_commands_allowed=false`, `auto_start_allowed=false`), `INFOJSON` and `SMOKEJSON` CI evidence hints, artifact retention evidence (`artifact_root="c/"`, `artifact_path_hint="c/81/"`, `retention_days=30`, `github_artifact_upload_attempted=false`, `production_window_allowed=false`), fixture inventory, version evidence, no-runtime-write boundary, failure taxonomy digest evidence, operator-window identity-neutral proof, rollback evidence sample, runtime artifact bundle manifest, restore compatibility handoff sample, restore dry-run operator package, artifact digest compatibility matrix, release artifact digest package, restore drill evidence, restore handoff checklist, restore evidence retention, retained restore artifact digest, restore approval boundary, restore boundary smoke manifest, runtime smoke evidence, retention provenance checks (`expected_binary_provenance_digest="fnv1a64:05de09a5ea28b666"`, `check_digest="fnv1a64:9e97e1284756f0e3"`, `managed_audit_write_executed=false`), a retention provenance replay marker (`consumed_release_version="v84"`, `consumed_artifact_path_hint="c/84/"`, `consumed_check_digest="fnv1a64:357cc7e9eec3f223"`, `marker_digest="fnv1a64:5b19ed3789b3f799"`, `replay_executed=false`), a managed audit adapter restore boundary receipt (`consumed_release_version="v85"`, `consumed_artifact_path_hint="c/85/"`, `consumed_marker_digest="fnv1a64:1ea4570c967cfdb1"`, `receipt_digest="fnv1a64:7d0e1dbbfabd869e"`, `adapter_write_allowed=false`, `restore_execution_allowed=false`, `load_restore_compact_executed=false`), a managed audit adapter non-authoritative storage receipt (`consumed_release_version="v86"`, `consumed_artifact_path_hint="c/86/"`, `consumed_receipt_digest="fnv1a64:f39d8e3ef98654ea"`, `receipt_digest="fnv1a64:b528c09e1d2e919e"`, `managed_audit_store=false`, `storage_write_allowed=false`, `admin_commands_allowed=false`), a command dispatch quality receipt (`consumed_release_version="v87"`, `consumed_artifact_path_hint="c/87/"`, `consumed_receipt_digest="fnv1a64:111f0daf1283eab6"`, `receipt_digest="fnv1a64:1f14361070b229f2"`, `dispatch_split_applied=true`, `write_handler_changed=false`, `admin_handler_changed=false`, `wal_snapshot_restore_touched=false`, `behavior_changed=false`), an adapter shell non-storage guard receipt (`consumed_release_version="v88"`, `consumed_artifact_path_hint="c/88/"`, `consumed_receipt_digest="fnv1a64:4aa6d12fb067e2a6"`, `receipt_digest="fnv1a64:385f612f8ea585d0"`, `adapter_shell_storage_backend=false`, `storage_backend_allowed=false`, `local_dry_run_records_written=false`), and a managed audit external adapter non-participation receipt (`consumed_release_version="v89"`, `consumed_artifact_path_hint="c/89/"`, `consumed_receipt_digest="fnv1a64:76411286a0913dc8"`, `receipt_digest="fnv1a64:0dfb07cd2f8de289"`, `external_adapter_storage_backend=false`, `participates_in_external_adapter=false`, `credential_read_allowed=false`, `migration_execution_allowed=false`).

`fixtures/release/release-artifact-digest-package.json` records binary, WAL, Snapshot, and fixture digest placeholders plus restore drill commands for Node v170; it uses `CHECKJSON LOAD data/release-artifact-drill.snap`, `CHECKJSON COMPACT`, and `CHECKJSON SETNXEX release:token 30 value` only as read-only package checks, and `GET release:token` must remain `(nil)`. `fixtures/release/restore-drill-evidence.json` records the v70 restore drill target, digest comparison placeholder, operator confirmation, and CHECKJSON dry-run commands for Node v173; it uses `CHECKJSON LOAD data/restore-drill.snap`, `CHECKJSON COMPACT`, and `CHECKJSON SETNXEX restore:drill-token 30 value` only as read-only drill checks, and `GET restore:drill-token` must remain `(nil)`. `fixtures/release/restore-handoff-checklist.json` records the v71 restore handoff checklist for Node v175; it uses `CHECKJSON LOAD data/handoff-restore.snap`, `CHECKJSON COMPACT`, and `CHECKJSON SETNXEX restore:handoff-token 30 value` only as read-only risk confirmation, and `GET restore:handoff-token` must remain `(nil)`. `fixtures/release/restore-evidence-retention.json` records the v72 restore evidence retention fields for Node v178; it uses `CHECKJSON LOAD data/retention-restore.snap`, `CHECKJSON COMPACT`, and `CHECKJSON SETNXEX restore:retention-token 30 value` only as retained risk evidence, and `GET restore:retention-token` must remain `(nil)`. `fixtures/release/retained-restore-artifact-digest.json` records the v73 retained restore artifact digest fields for Node v180; it uses `CHECKJSON LOAD data/retained-digest-restore.snap`, `CHECKJSON COMPACT`, and `CHECKJSON SETNXEX restore:digest-token 30 value` only as retained digest evidence, and `GET restore:digest-token` must remain `(nil)`. `fixtures/release/restore-approval-boundary.json` records the v74 restore approval boundary fields for Node v182; it uses `CHECKJSON LOAD data/approval-boundary-restore.snap`, `CHECKJSON COMPACT`, and `CHECKJSON SETNXEX restore:approval-token 30 value` only as approval-boundary risk evidence, and `GET restore:approval-token` must remain `(nil)`. `fixtures/release/restore-boundary-smoke-manifest.json` records the v75 real-read smoke manifest for Node v185; it uses `CHECKJSON LOAD data/real-read-boundary-restore.snap`, `CHECKJSON COMPACT`, and `CHECKJSON SETNXEX restore:real-read-token 30 value` only as real-read risk evidence, and `GET restore:real-read-token` must remain `(nil)`. `fixtures/release/runtime-smoke-evidence.json` records the v90 `SMOKEJSON` read target for Node v223; it uses `SMOKEJSON`, `INFOJSON`, `STORAGEJSON`, `HEALTH`, and `GET restore:real-read-token` as read-only runtime evidence, adds `live_read_session` hints (`session_id_echo="mini-kv-live-read-v90"`, `read_command_list_digest="fnv1a64:5bef33f2fbe65cc5"`, `write_commands_allowed=false`, `auto_start_allowed=false`), failure taxonomy digest `fnv1a64:f92fcba55feb26a2`, operator-window/CI/artifact-retention hints, retention provenance check digest `fnv1a64:9e97e1284756f0e3`, replay marker digest `fnv1a64:5b19ed3789b3f799`, managed audit adapter restore boundary digest `fnv1a64:7d0e1dbbfabd869e`, managed audit adapter non-authoritative storage digest `fnv1a64:b528c09e1d2e919e`, command dispatch quality digest `fnv1a64:1f14361070b229f2`, adapter shell non-storage guard digest `fnv1a64:385f612f8ea585d0`, and external adapter non-participation digest `fnv1a64:0dfb07cd2f8de289`; `GET restore:real-read-token` must remain `(nil)` and mini-kv must not become an external adapter storage backend or read managed audit credentials.

`fixtures/release/runtime-smoke-evidence.json` and `fixtures/release/verification-manifest.json` also keep the historical managed audit adapter non-authoritative storage receipt for Node v217, the v88 command dispatch quality receipt for Node v219, and the v89 adapter shell non-storage guard receipt for Node v221, then add the v90 external adapter non-participation receipt for Node v223. The new receipt consumes the v89 adapter shell guard digest `fnv1a64:76411286a0913dc8`, publishes the v90 external adapter non-participation digest `fnv1a64:0dfb07cd2f8de289`, and proves this version only exposes read-only runtime evidence while leaving external adapter storage backend selection, credential reads, migration execution, adapter participation, local dry-run writes, managed audit writes, write handlers, admin handlers, WAL, snapshot, restore, and Java order authority untouched.

`command_tests` compares both CHECKJSON fixtures with real command output and checks the TTL token fixture index, `readonly_fixture_tests` compares the read-only fixture package with real `CHECKJSON`, `INFOJSON`, and `STATSJSON` output plus the runtime read field guide, `recovery_evidence_tests` and `recovery_fixture_index_tests` protect restart evidence, `ttl_token_recovery_tests` exercises TTL recovery, and the release tests protect the manifest, rollback evidence, bundle manifest, restore handoff, dry-run operator package, digest matrix, release artifact digest package, restore drill evidence, restore handoff checklist, restore evidence retention, retained restore artifact digest, restore approval boundary, restore boundary smoke, runtime smoke no-execution boundaries, SMOKEJSON live-read session hints, failure taxonomy digest/sample fields, operator-window identity-neutral proof, CI evidence hints, artifact retention evidence, managed audit restore-boundary receipts, managed audit non-authoritative storage receipts, command dispatch quality receipts, adapter shell non-storage guard receipts, and external adapter non-participation receipts.

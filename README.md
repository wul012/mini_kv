...
Version 56 is a runnable in-memory KV service with injected build version metadata, command catalog metadata, bounded JSON key inventory, digest-aware structured command risk explanation, read-only write-command execution contracts, stable CHECKJSON write/read fixture samples, read-only storage evidence, and indexed restart recovery evidence:
...
- Repository fixture `fixtures/recovery/index.json` lists recovery evidence fixtures, paths, digest, consumer hints, and read-only boundaries for downstream smoke tests
...
1. Let Node v103 consume `fixtures/recovery/index.json` and `fixtures/recovery/restart-recovery-evidence.json` alongside Java evidence explanation.

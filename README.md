...
Version 93 is a runnable in-memory KV service with injected build version metadata, command catalog metadata, bounded JSON key inventory, digest-aware structured command risk explanation, read-only write-command execution contracts, a short-TTL token primitive with recovery evidence, release verification, runtime artifact rollback, runtime artifact bundle, restore compatibility handoff, restore dry-run operator package, artifact digest compatibility matrix, release artifact digest package, restore drill evidence, restore handoff checklist, restore evidence retention, retained restore artifact digest, restore approval boundary, restore boundary smoke manifests, structured runtime smoke evidence, SMOKEJSON failure taxonomy digest verification for imported real-read window results, an operator-window identity-neutral proof for archived real-read window checks, read-only CI/evidence hints for Node v201 manifest verification, read-only artifact retention evidence for Node v203 cross-project retention gate checks, live-read session hints for Node v205 real-read execution packet preparation, binary provenance and retention provenance hints for managed audit preparation, a managed audit adapter restore boundary receipt, a managed audit adapter non-authoritative storage receipt for Node v217 readiness gating, a v88 command dispatch quality receipt for Node v219 implementation precheck, a v89 adapter shell non-storage guard receipt for Node v221 local adapter candidate dry-run, a v90 managed audit external adapter non-participation receipt for Node v223 connection readiness review, and a v93 managed audit sandbox adapter non-participation receipt for Node v225 sandbox adapter dry-run package, plus contract-preserving formatter splits for managed audit receipts (`include/minikv/managed_audit_receipts.hpp`, `src/managed_audit_receipts.cpp`) and runtime evidence receipts (`include/minikv/runtime_evidence_receipts.hpp`, `src/runtime_evidence_receipts.cpp`), stable CHECKJSON write/read fixture samples, self-describing INFOJSON/STATSJSON/SMOKEJSON read-only runtime evidence, an INFOJSON/STATSJSON field guide for release evidence review, read-only storage evidence, and indexed restart recovery evidence with restart/replay cost and retention boundaries:
...
- Repository fixture `fixtures/release/runtime-smoke-evidence.json` records the v93 SMOKEJSON real-read command target for Node v225 managed audit sandbox adapter dry-run package: it uses `SMOKEJSON`, `INFOJSON`, `STORAGEJSON`, `HEALTH`, and `GET restore:real-read-token` as read-only runtime evidence, adds live-read session id echo `mini-kv-live-read-v93`, binary provenance digest `fnv1a64:077c4510bc08fe58`, retention provenance digest `fnv1a64:123b13c1e2e31ba2`, replay marker digest `fnv1a64:b0daa0cec61f2931`, managed audit adapter restore/non-authoritative receipts, the v88 command dispatch receipt, the v89 adapter shell non-storage guard receipt, the v90 external adapter non-participation receipt, and the v93 sandbox adapter non-participation receipt `fnv1a64:a09d9fc409ab38ec` without executing LOAD, COMPACT, SETNXEX, restore, CI upload, production window opening, managed audit writes, adapter storage writes, credential reads, external or sandbox adapter migration, sandbox dry-run record writes, or sandbox managed audit state writes
- Repository fixtures `fixtures/release/runtime-smoke-evidence.json` and `fixtures/release/verification-manifest.json` keep the v90 external adapter non-participation receipt and add the v93 sandbox adapter non-participation receipt with `sandbox_adapter_storage_backend=false`, `participates_in_sandbox_adapter=false`, `credential_value_read_allowed=false`, `production_credential_read_allowed=false`, `schema_migration_execution_allowed=false`, `sandbox_schema_authoritative=false`, `sandbox_managed_audit_state_write_allowed=false`, `sandbox_dry_run_records_written=false`, `managed_audit_write_executed=false`, `restore_execution_allowed=false`, `load_restore_compact_executed=false`, and `order_authoritative=false`

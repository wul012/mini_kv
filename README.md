...
- Repository fixture `fixtures/release/verification-manifest.json` documents the CMake configure/build, CTest, read-only command smoke, fixture inventory, version evidence, rollback evidence sample, runtime artifact bundle manifest, restore compatibility handoff sample, restore dry-run operator package, artifact digest compatibility matrix, release artifact digest package, restore drill evidence, restore handoff checklist, restore evidence retention, and retained restore artifact digest expected before Node v180 consumes mini-kv approval prerequisite evidence
...
2. Let Java v64 and mini-kv v73 progress in parallel on release operator signoff and retained restore artifact digest fixtures without executing deployment, rollback SQL, LOAD, COMPACT, SETNXEX, or restore.
3. Let Node v180 wait for Java v64, mini-kv v73, and Node v179, then create the approval decision prerequisite gate without creating an approval decision.
4. Let Node v181 wait for Node v180, then create an approval ledger dry-run envelope without writing the real ledger or publishing.
5. Let Java v65 and mini-kv v74 progress in parallel on rollback approver and restore approval boundary fixtures before Node v182 rehearses the release approval decision packet.

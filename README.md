...
- Repository fixture `fixtures/release/runtime-smoke-evidence.json` records the v96 SMOKEJSON real-read command target for Node v229 manual sandbox connection packet verification: it uses `SMOKEJSON`, `INFOJSON`, `STORAGEJSON`, `HEALTH`, and `GET restore:real-read-token` as read-only runtime evidence, adds live-read session id echo `mini-kv-live-read-v96`, binary provenance digest `fnv1a64:c9da016a60b4a58e`, retention provenance digest `fnv1a64:e8df420be507be3e`, replay marker digest `fnv1a64:0e8a532e42d289d5`, managed audit adapter restore/non-authoritative receipts, the v88 command dispatch receipt, the v89 adapter shell non-storage guard receipt, the v90 external adapter non-participation receipt, the v95 sandbox adapter non-participation receipt `fnv1a64:e3693d38283c37e2`, and the v96 sandbox connection receipt echo marker `fnv1a64:b9fc556875ea625b` without executing LOAD, COMPACT, SETNXEX, restore, CI upload, production window opening, managed audit writes, adapter storage writes, credential value reads, external or sandbox adapter migration, schema rehearsal, sandbox connection execution, sandbox dry-run record writes, or sandbox managed audit state writes
- Repository fixtures `fixtures/release/runtime-smoke-evidence.json` and `fixtures/release/verification-manifest.json` keep the v90 external adapter non-participation receipt, the v95 sandbox adapter non-participation receipt, and the v96 sandbox connection receipt echo marker with `connection_execution_allowed=false`, `credential_value_read_allowed=false`, `schema_migration_execution_allowed=false`, `managed_audit_write_allowed=false`, `sandbox_adapter_storage_backend=false`, `participates_in_sandbox_connection=false`, `sandbox_managed_audit_state_write_allowed=false`, `node_auto_start_allowed=false`, `restore_execution_allowed=false`, `load_restore_compact_executed=false`, and `order_authoritative=false`

...
- Runtime `EXPLAINJSON command` explains a command's parser validity, key target, TTL sensitivity, mutation risk, and WAL risk without executing it
...
1. Add external control-plane smoke notes once Node consumes `EXPLAINJSON` in its operation execution preview.

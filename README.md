...
Version 58 is a runnable in-memory KV service with injected build version metadata, command catalog metadata, bounded JSON key inventory, digest-aware structured command risk explanation, read-only write-command execution contracts, stable CHECKJSON write/read fixture samples, INFOJSON/STATSJSON read-only fixture samples, read-only storage evidence, and indexed restart recovery evidence with restart/replay cost and retention boundaries:
...
- Repository fixture `fixtures/readonly/index.json` groups CHECKJSON, INFOJSON, and STATSJSON read-only samples for Node v152/v153 production pass evidence preparation
...
1. Let Node v152 consume Java v49 and mini-kv v58 read-only evidence references while keeping `upstreamEvidenceReady=false` until both upstream versions are complete.
2. Let Node v153 turn the read-only evidence package into an operator runbook for a manual real-read smoke window.

...
- Runtime `SMOKEJSON` combines version, identity, storage, WAL, command metrics, connection stats, real-read command list, forbidden command list, and no-write diagnostics into one read-only JSON object for Node v191 real-read adapter rehearsal
...
- Repository fixture `fixtures/release/verification-manifest.json` documents the CMake configure/build, CTest, read-only SMOKEJSON runtime smoke, fixture inventory, version evidence, rollback evidence sample, runtime artifact bundle manifest, restore compatibility handoff sample, restore dry-run operator package, artifact digest compatibility matrix, release artifact digest package, restore drill evidence, restore handoff checklist, restore evidence retention, retained restore artifact digest, restore approval boundary, restore boundary smoke manifest, and runtime smoke evidence expected before Node v191 consumes mini-kv real-read adapter rehearsal evidence
...
- Repository fixture `fixtures/release/runtime-smoke-evidence.json` records the v76 SMOKEJSON real-read command target, Node v191 consumption boundary, forbidden command list, and no-write proof without executing LOAD, COMPACT, SETNXEX, or restore

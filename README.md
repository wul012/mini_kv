...
Version 57 is a runnable in-memory KV service with injected build version metadata, command catalog metadata, bounded JSON key inventory, digest-aware structured command risk explanation, read-only write-command execution contracts, stable CHECKJSON write/read fixture samples, read-only storage evidence, and indexed restart recovery evidence with restart/replay cost and retention boundaries:
...
1. Let Node v107 consume `fixtures/recovery/index.json` and `fixtures/recovery/restart-recovery-evidence.json` alongside Java v48 evidence enhancements.
2. Consider additional recovery samples only after Node proves it needs more failure shapes for diagnostics.

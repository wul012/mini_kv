...
- Runtime `SMOKEJSON` combines version, identity, storage, WAL, command metrics, connection stats, real-read command list, forbidden command list, no-write diagnostics, failure taxonomy, taxonomy digest, verification sample, and operator-window identity-neutral proof into one read-only JSON object for Node v200 archived real-read window checks
...
- Repository fixture `fixtures/release/verification-manifest.json` documents the CMake configure/build, CTest, read-only SMOKEJSON runtime smoke, failure taxonomy digest, operator-window identity-neutral proof, fixture inventory, version evidence, rollback evidence sample, runtime artifact bundle manifest, restore compatibility handoff sample, restore dry-run operator package, artifact digest compatibility matrix, release artifact digest package, restore drill evidence, restore handoff checklist, restore evidence retention, retained restore artifact digest, restore approval boundary, restore boundary smoke manifest, and runtime smoke evidence expected before Node v200 archives mini-kv real-read window evidence
...
- Repository fixture `fixtures/release/runtime-smoke-evidence.json` records the v79 SMOKEJSON real-read command target, Node v200 archive boundary, forbidden command list, failure taxonomy digest, verification sample, operator-window proof, and no-write proof without executing LOAD, COMPACT, SETNXEX, or restore

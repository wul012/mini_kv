...
Version 63 is a runnable in-memory KV service with injected build version metadata, command catalog metadata, bounded JSON key inventory, digest-aware structured command risk explanation, read-only write-command execution contracts, a short-TTL token primitive with recovery evidence, a release verification manifest, stable CHECKJSON write/read fixture samples, self-describing INFOJSON/STATSJSON read-only runtime evidence, an INFOJSON/STATSJSON field guide for release evidence review, read-only storage evidence, and indexed restart recovery evidence with restart/replay cost and retention boundaries:
...
- Repository fixture `fixtures/release/verification-manifest.json` documents the CMake configure/build, CTest, read-only command smoke, fixture inventory, and version evidence expected before Node v162 consumes mini-kv release verification evidence
...
1. Let Java v54 and mini-kv v63 progress in parallel on release verification manifests without changing runtime business semantics or adding mini-kv runtime commands.
2. Let Node v162 wait for Java v54 and mini-kv v63 evidence, then create a cross-project release verification intake gate that only consumes archived evidence.

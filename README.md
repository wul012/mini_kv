...
Version 52 is a runnable in-memory KV service with injected build version metadata, command catalog metadata, bounded JSON key inventory, digest-aware structured command risk explanation, read-only write-command execution contracts, and stable CHECKJSON fixture samples:
...
Stable sample responses live under `fixtures/checkjson/`. `fixtures/checkjson/set-orderops-write-contract.json` is the exact no-WAL response for `CHECKJSON SET orderops:1 value`, and `command_tests` compares the fixture with the real command output to catch drift before Node smoke tests consume it.
...
1. Add external control-plane smoke notes once Node consumes `fixtures/checkjson/set-orderops-write-contract.json` in fixture-driven smoke.

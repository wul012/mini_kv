...
- Repository fixture `fixtures/release/runtime-smoke-evidence.json` records the v101 SMOKEJSON read target for Node v237 manual sandbox connection readiness gate: it uses `SMOKEJSON`, `INFOJSON`, `STORAGEJSON`, `HEALTH`, and `GET restore:real-read-token` as read-only runtime evidence, adds live-read session id echo `mini-kv-live-read-v101`, binary provenance digest `fnv1a64:6b0e9514847a3e7a`, retention provenance digest `fnv1a64:ef2742262316a25a`, replay marker digest `fnv1a64:3dcfdb4fc3986949`, managed audit adapter restore/non-authoritative receipts, the v88 command dispatch receipt, the v89 adapter shell non-storage guard receipt, the v90 external adapter non-participation receipt, the v95 sandbox adapter non-participation receipt `fnv1a64:eea9770f26d0bade`, the v96 sandbox connection receipt echo marker `fnv1a64:259cb68de0117847`, and the no-start guard receipt `fnv1a64:f5c9fa97c073ba43` that v101 continues in its current evidence chain without executing LOAD, COMPACT, SETNXEX, restore, CI upload, production window opening, managed audit writes, adapter storage writes, credential value reads, external or sandbox adapter migration, schema rehearsal, sandbox connection execution, Node/Java/mini-kv auto-start, sandbox dry-run record writes, or sandbox managed audit state writes

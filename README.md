...
Version 61 is a runnable in-memory KV service with injected build version metadata, command catalog metadata, bounded JSON key inventory, digest-aware structured command risk explanation, read-only write-command execution contracts, a short-TTL token primitive, stable CHECKJSON write/read fixture samples, self-describing INFOJSON/STATSJSON read-only runtime evidence, an INFOJSON/STATSJSON field guide for release evidence review, read-only storage evidence, and indexed restart recovery evidence with restart/replay cost and retention boundaries:
...
1. Let Java v52 and mini-kv v61 progress in parallel on the idempotency / TTL-token vertical slice without connecting mini-kv to Java's transaction chain.
2. Let Node v160 wait for Java v52 and mini-kv v61 evidence, then create an idempotency vertical readiness review with upstream writes disabled by default.

...
- Runtime `INFOJSON` and `SMOKEJSON` expose the historical `managed_audit_adapter_non_authoritative_storage_receipt` evidence for Node v217, the v88 `command_dispatch_quality_receipt` for Node v219, the v89 `adapter_shell_non_storage_guard_receipt` for Node v221, the v90 `managed_audit_external_adapter_non_participation_receipt` for Node v223, the v95 `managed_audit_sandbox_adapter_non_participation_receipt` for Node v225, the v96 `managed_audit_sandbox_connection_receipt_echo_marker` for Node v229, and the historical no-start guard receipt for Node v231 while v99 keeps the same no-start boundary in its current evidence chain: mini-kv remains read-only runtime evidence, is not the managed audit store, is not the adapter shell, external adapter storage backend, sandbox audit storage backend, or sandbox connection participant, does not read managed audit, sandbox, or production credential values, does not execute external or sandbox adapter migrations or schema rehearsal, does not write local dry-run records or sandbox managed audit state, does not auto-start Node/Java, does not allow admin/restore execution, does not change write/admin/WAL/snapshot/restore handlers, and is not Java order authority
- Runtime managed audit receipt formatting and digest generation now live in `include/minikv/managed_audit_receipts.hpp` and `src/managed_audit_receipts.cpp`; INFOJSON/SMOKEJSON response formatting now flows through the runtime response formatter module, and the split does not change write/admin/WAL/snapshot/restore behavior or runtime JSON boundaries
...
- Repository fixture `fixtures/release/runtime-smoke-evidence.json` records the v99 SMOKEJSON read target for Node v234 manual sandbox connection blocked execution rehearsal: it uses `SMOKEJSON`, `INFOJSON`, `STORAGEJSON`, `HEALTH`, and `GET restore:real-read-token` as read-only runtime evidence, adds live-read session id echo `mini-kv-live-read-v99`, binary provenance digest `fnv1a64:68ec9a2a032906f4`, retention provenance digest `fnv1a64:56b28a6190be9fe9`, replay marker digest `fnv1a64:03afe1be1ce63765`, managed audit adapter restore/non-authoritative receipts, the v88 command dispatch receipt, the v89 adapter shell non-storage guard receipt, the v90 external adapter non-participation receipt, the v95 sandbox adapter non-participation receipt `fnv1a64:e718f4de6ca49014`, the v96 sandbox connection receipt echo marker `fnv1a64:0d5212cff01975af`, and the no-start guard receipt `fnv1a64:1f09161efd809c33` that v99 continues in its current evidence chain without executing LOAD, COMPACT, SETNXEX, restore, CI upload, production window opening, managed audit writes, adapter storage writes, credential value reads, external or sandbox adapter migration, schema rehearsal, sandbox connection execution, Node/Java/mini-kv auto-start, sandbox dry-run record writes, or sandbox managed audit state writes

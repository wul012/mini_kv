...
- Runtime `SMOKEJSON` combines version, identity, storage, WAL, command metrics, connection stats, real-read command list, forbidden command list, no-write diagnostics, failure taxonomy, taxonomy digest, and verification sample into one read-only JSON object for Node v196 imported real-read window result verification
...
- Repository fixture `fixtures/release/verification-manifest.json` documents the CMake configure/build, CTest, read-only SMOKEJSON runtime smoke, failure taxonomy digest, fixture inventory, version evidence, rollback evidence sample, runtime artifact bundle manifest, restore compatibility handoff sample, restore dry-run operator package, artifact digest compatibility matrix, release artifact digest package, restore drill evidence, restore handoff checklist, restore evidence retention, retained restore artifact digest, restore approval boundary, restore boundary smoke manifest, and runtime smoke evidence expected before Node v196 imports mini-kv real-read window result evidence
...
- Repository fixture `fixtures/release/runtime-smoke-evidence.json` records the v78 SMOKEJSON real-read command target, Node v196 consumption boundary, forbidden command list, failure taxonomy digest, verification sample, and no-write proof without executing LOAD, COMPACT, SETNXEX, or restore

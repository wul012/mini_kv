...
Version 62 is a runnable in-memory KV service with injected build version metadata, command catalog metadata, bounded JSON key inventory, digest-aware structured command risk explanation, read-only write-command execution contracts, a short-TTL token primitive with recovery evidence, stable CHECKJSON write/read fixture samples, self-describing INFOJSON/STATSJSON read-only runtime evidence, an INFOJSON/STATSJSON field guide for release evidence review, read-only storage evidence, and indexed restart recovery evidence with restart/replay cost and retention boundaries:
...
1. Let Java v53 and mini-kv v62 progress in parallel on the idempotency abstraction / TTL recovery evidence slice without connecting mini-kv to Java's transaction chain.
2. Let Node v161 wait for Java v53 and mini-kv v62 evidence, then create a controlled idempotency drill runbook with read-only or dry-run defaults.

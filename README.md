...
- Repository fixture `fixtures/release/runtime-smoke-evidence.json` records the v100 SMOKEJSON read target for Node v235 manual sandbox connection precondition intake: it uses `SMOKEJSON`, `INFOJSON`, `STORAGEJSON`, `HEALTH`, and `GET restore:real-read-token` as read-only runtime evidence, adds live-read session id echo `mini-kv-live-read-v100`, binary provenance digest `fnv1a64:4b6d333035bfbba0`, retention provenance digest `fnv1a64:5cd74c21104d0f66`, replay marker digest `fnv1a64:298a3e48980ac14b`, managed audit adapter restore/non-authoritative receipts, the v88 command dispatch receipt, the v89 adapter shell non-storage guard receipt, the v90 external adapter non-participation receipt, the v95 sandbox adapter non-participation receipt `fnv1a64:3a5d3e7f4a3092f6`, the v96 sandbox connection receipt echo marker `fnv1a64:a5ea80910cb99931`, and the no-start guard receipt `fnv1a64:5530a3cc527cd8eb` that v100 continues in its current evidence chain without executing LOAD, COMPACT, SETNXEX, restore, CI upload, production window opening, managed audit writes, adapter storage writes, credential value reads, external or sandbox adapter migration, schema rehearsal, sandbox connection execution, Node/Java/mini-kv auto-start, sandbox dry-run record writes, or sandbox managed audit state writes

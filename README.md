...
- v1624: turns the v1623 plain-language orientation examples into an executable CTest surface by adding `project_orientation_examples_tests`, which drives `SET order:100 paid`, `GET order:100`, `INFOJSON`, `CHECKJSON LOAD data/prod.snap`, and `SMOKEJSON` through the real `Store` + `CommandProcessor` path, updates the guide contract so those examples remain documented, and proves `CHECKJSON` still explains dangerous commands without loading snapshots, replacing the store, starting services, routing writes, touching WAL, or granting execution authority.

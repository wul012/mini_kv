...
- Repository fixture `fixtures/release/runtime-smoke-evidence.json` records the v89 SMOKEJSON real-read command target for Node v221 managed audit local adapter candidate dry-run: it uses `SMOKEJSON`, `INFOJSON`, `STORAGEJSON`, `HEALTH`, and `GET restore:real-read-token` as read-only runtime evidence, adds live-read session id echo `mini-kv-live-read-v89`, binary provenance digest `fnv1a64:6a020918524947fa`, retention provenance digest `fnv1a64:170d7bba9c9c3c7f`, replay marker digest `fnv1a64:6711cd4fe22feae9`, managed audit adapter restore/non-authoritative receipts, the v88 command dispatch receipt, and the v89 adapter shell non-storage guard receipt without executing LOAD, COMPACT, SETNXEX, restore, CI upload, production window opening, managed audit writes, adapter storage writes, or local dry-run record writes
- Repository fixtures `fixtures/release/runtime-smoke-evidence.json` and `fixtures/release/verification-manifest.json` now carry the v89 adapter shell non-storage guard receipt with `adapter_shell_storage_backend=false`, `storage_backend_allowed=false`, `local_dry_run_records_written=false`, `managed_audit_write_executed=false`, `restore_execution_allowed=false`, `load_restore_compact_executed=false`, and `order_authoritative=false`

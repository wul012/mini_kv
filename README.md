...
- v1625: establishes the runtime-receipts consolidation design gate before any formatter refactor: `docs/receipts-consolidation-note.md` freezes the 28-file / 788,811-byte runtime receipt census, identifies the 26-file credential-resolver family as the first target, specifies an ordered zero-dependency JSON builder and a fixture-subobject byte-parity harness, and adds `receipts_consolidation_note_contract` so future migrations cannot skip the necessity proof, fixture oracle, phased slices, or no-credential/no-network/no-write/no-execution boundaries.
## Current version

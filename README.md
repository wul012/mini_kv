...
`fixtures/release/verification-manifest.json` is the v75 release verification manifest for Node v185 real-read rehearsal intake: it lists expected CMake configure/build, CTest, targeted tests, read-only command smoke, fixture inventory, version evidence, no-runtime-command boundary, rollback evidence sample, runtime artifact bundle manifest, restore compatibility handoff sample, restore dry-run operator package, artifact digest compatibility matrix, release artifact digest package, restore drill evidence, restore handoff checklist, restore evidence retention, retained restore artifact digest, restore approval boundary, and restore boundary smoke manifest.

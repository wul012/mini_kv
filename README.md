...
- Repository fixture `fixtures/release/runtime-smoke-evidence.json` records the v98 SMOKEJSON read target for Node v233 manual sandbox connection rehearsal packet review: it uses `SMOKEJSON`, `INFOJSON`, `STORAGEJSON`, `HEALTH`, and `GET restore:real-read-token` as read-only runtime evidence, adds live-read session id echo `mini-kv-live-read-v98`, binary provenance digest `fnv1a64:4be577ac4f8347c6`, retention provenance digest `fnv1a64:3397146bc78ee26f`, replay marker digest `fnv1a64:f8c7f073fdddb289`, managed audit adapter restore/non-authoritative receipts, the v88 command dispatch receipt, the v89 adapter shell non-storage guard receipt, the v90 external adapter non-participation receipt, the v95 sandbox adapter non-participation receipt `fnv1a64:9e89d248ca215706`, the v96 sandbox connection receipt echo marker `fnv1a64:c88b0bf6b974ac6b`, and the no-start guard receipt `fnv1a64:431780d3772a52a5` that v98 continues in its current evidence chain without executing LOAD, COMPACT, SETNXEX, restore, CI upload, production window opening, managed audit writes, adapter storage writes, credential value reads, external or sandbox adapter migration, schema rehearsal, sandbox connection execution, Node/Java/mini-kv auto-start, sandbox dry-run record writes, or sandbox managed audit state writes

...
- v1613: hardens the K2 Ubuntu coverage report generation after v1612 proved the coverage build and 334 CTest path but `gcovr 7.0` failed on GCC/gcov negative branch-hit output in `src/command.cpp`; the coverage job now passes `--gcov-ignore-parse-errors negative_hits.warn_once_per_file`, the CI contract test protects that parser policy, and the version records the issue as a toolchain deviation while preserving runtime commands, WAL, snapshots, TCP/RESP, fixtures, archive roots, write authority, router authority, and execution authority unchanged.
- v1612: starts production-excellence K2 coverage baselining by adding `MINIKV_COVERAGE` for GCC/Clang gcov-compatible builds, wiring a required Ubuntu coverage CI job that configures, builds, runs full CTest, generates a `gcovr` core-module report artifact for store/command/WAL/snapshot/RESP files, testing the CI contract, and making local Windows MinGW coverage fail fast with a clear gcov sidecar path-limit message while preserving default builds, runtime commands, WAL, snapshots, TCP/RESP, fixtures, archive roots, write authority, router authority, and execution authority unchanged.
- v1611: completes production-excellence K1 sanitizer enforcement by removing the diagnostic `continue-on-error` escape hatch from the Ubuntu sanitizer CI job after v1610 proved the ASan/UBSan build and full CTest pass on GitHub Ubuntu, updating CI contract tests to reject future silent weakening, and keeping the same no-runtime-command/no-router/no-write/no-WAL/no-execution boundary.

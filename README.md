...
- v1641: migrates the signed-human approval artifact contract receipt's closed-boundary flag block onto the internal ordered JSON builder, matching the v1639 credential-handle and v1640 endpoint-handle slices without broadening either one. The existing v1638 single spelling waiver remains exactly scoped to the frozen fixture text `Node v314\u0027s non-secret contract` versus the current runtime surface `Node v314's non-secret contract`; no fixture bytes are rewritten. Focused tests prove the signed-human canonicalized fixture parity, SMOKEJSON, runtime smoke evidence, and release manifest surfaces remain stable, and full local CTest stays green at 344/344. This still does not claim full approval-family migration, and it does not open router/write/WAL/network/credential/raw-endpoint/schema/restore/load/compact/auto-start/audit/order authority.
- v1640: migrates the endpoint-handle allowlist approval contract receipt's closed-boundary flag block onto the same internal ordered JSON builder pattern used in v1639. The repeated endpoint/allowlist/raw-endpoint/no-network/no-write/no-execution boolean tail is now represented as ordered `OrderedJsonField` entries, rendered through `json_object`, and embedded back into the historical formatter position. The v1638 parity oracle, the endpoint-handle receipt test, SMOKEJSON, runtime smoke evidence, release manifest checks, and full local CTest prove fixture bytes, field order, digest meaning, no-router/no-write/no-WAL/no-network/no-credential, and execution-authority boundaries remain unchanged. This version still does not rewrite fixtures or claim signed-human/full approval-family migration.
- v1639: migrates the credential-handle approval contract receipt's closed-boundary flag block onto the internal ordered JSON builder while preserving the existing fixture/formatter byte parity. The highest-risk repeated boolean tail is now represented as ordered `OrderedJsonField` entries and rendered through `json_object`, then embedded back into the existing formatter at the same position; `runtime_receipt_json_builder_tests`, the credential-handle receipt test, SMOKEJSON, runtime smoke evidence, and release manifest checks prove there is no field-order, digest, boundary, no-router/no-write/no-WAL/no-network/no-credential, or execution-authority drift. This version does not rewrite fixtures and does not claim the whole approval-contract formatter family is migrated.
- v1638: adds the approval-contract Slice 2 migration baseline before changing another formatter. `runtime_receipt_json_builder_tests` now locks the signed-human approval artifact, credential-handle approval, and endpoint-handle allowlist approval receipt formatters against their frozen fixture subobjects. Credential-handle and endpoint-handle are byte-for-byte equal to fixtures; signed-human has exactly one already-existing runtime-surface spelling difference (`Node v314\u0027s` in the frozen fixture versus `Node v314's` in the current formatter), with identical field count, canonicalized bytes, and unchanged read-only/no-router/no-write/no-WAL/no-network/no-credential/no-execution boundaries. No formatter is migrated in this guardrail version; it prepares the next builder slice with executable parity evidence.
- v1637: closes the no-network runtime receipt drift before scaling the remaining receipt migrations. The current public formatter spelling `Node v323's no-network fixture contract` is now the canonical runtime surface, while the committed fixture spelling `Node v323\u0027s no-network fixture contract` stays frozen and is not rewritten. `runtime_receipt_json_builder_tests` allows exactly that one historical spelling normalization and fails on any second `\u0027`, field-count drift, digest drift, or no-network/no-write/WAL/credential/execution boundary drift. `runtime_credential_resolver_no_network_safety_fixture_receipts.cpp` now uses the internal ordered JSON builder, so abort/rollback and no-network are the first two builder-backed receipt formatters; no fixture, manifest, public command output, router, write path, WAL behavior, network access, credential value, or execution authority is expanded.
- v1636: refreshes the OSFS coursework final package after the v1634/v1635 substance landed. `课程设计交付/v1636-osfs-final/` now contains a regenerated DOCX/PDF report, four updated diagrams, UTF-8 real CLI transcript, executable demo script, demo guide, submission checklist, and requirement-to-evidence matrix that upgrades single-indirect blocks, read-only FSCK, and root-only user administration from disclosed gaps to implemented evidence. The package proves a 4291-byte file crossing the direct-to-indirect boundary, `FSCK OK` before and after user management, Bob's permission and USERADD denial, root-created Carol with password reset, and clean visual PDF rendering; v1633 is retained only as superseded history. This is documentation/package closure for the independent OSFS course layer and still does not add KV/WAL/TCP routes, Node/Java authority, group descriptors, auto-repair FSCK, crash logging, or deeper directory trees.
- v1635: adds the OSFS read-only consistency checker and real on-disk user administration for the coursework capacity extension. `FSCK` now verifies superblock geometry, inode/block bitmap reachability, direct/indirect file block counts, user-table/MFD/UFD alignment, and directory ownership without auto-repair or mutation; `USERADD` is root-only and allocates a persistent UFD, while `PASSWD [user] password` supports root resets and self-service password changes. Tests cover clean and corrupted images, old-password rejection, wrong-user permission denial, full user-table failure, persistence after reopen, focused OSFS smoke, and full local CTest at 344/344. This remains an OSFS-only course layer change: no KV/WAL/TCP behavior, Node/Java route, execution authority, double/triple indirect block, crash log, or final-package regeneration is introduced.
- v1634: upgrades OSFS from an eight-direct-block teaching cap to disk-format v3 with one single-indirect block pointer per inode. File writes, range reads, truncation, deletion, free-block counters, persistence, and CLI smoke now cross the 4096-byte direct-to-indirect boundary while still rejecting writes beyond `8 + block_size / 4` data blocks; focused OSFS tests pass 2/2 and full local CTest passes 344/344. This version keeps FSCK, dynamic user management, double/triple indirect blocks, crash logs, KV/WAL/TCP behavior, Node/Java routes, and coursework final-package regeneration out of scope for the follow-up versions in `治理计划/v1634-osfs-capacity-extension-brief.md`.

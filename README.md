...
- v1653: freezes all thirteen outputs in the final nine pending owners before migration: eleven require raw equality, credential-resolver decision has one named field-order rule, and manual sandbox dry-run has one named three-metadata-field rule. The independent oracle also locks sizes, fields, digests, and ten closed boundaries; full local CTest remains 348/348.
- v1652: migrates four legacy resolver owners to ordered builders while keeping four exact fixtures/digests and local boundary ownership. The mechanical census reaches the Claude checkpoint at 18 builder-backed / 9 pending / 1 waiver; the documented line growth reflects explicit fields rather than a false shrink claim.

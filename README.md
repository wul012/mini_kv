...
Version 55 is a runnable in-memory KV service with injected build version metadata, command catalog metadata, bounded JSON key inventory, digest-aware structured command risk explanation, read-only write-command execution contracts, stable CHECKJSON write/read fixture samples, read-only storage evidence, and a restart recovery evidence sample:
...
1. Let Node v99 consume `fixtures/recovery/restart-recovery-evidence.json` alongside Java replay audit evidence.

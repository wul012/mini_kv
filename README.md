...
- Runtime `SMOKEJSON` combines version, identity, storage, WAL, command metrics, connection stats, real-read command list, forbidden command list, no-write diagnostics, and failure taxonomy into one read-only JSON object for Node v193 real-read adapter failure classification
...
- Repository fixture `fixtures/release/verification-manifest.json` documents the CMake configure/build, CTest, read-only SMOKEJSON runtime smoke, failure taxonomy, fixture inventory, version evidence, rollback evidence sample, runtime artifact bundle manifest, restore compatibility handoff sample, restore dry-run operator package, artifact digest compatibility matrix, release artifact digest package, restore drill evidence, restore handoff checklist, restore evidence retention, retained restore artifact digest, restore approval boundary, restore boundary smoke manifest, and runtime smoke evidence expected before Node v193 consumes mini-kv real-read adapter failure classification evidence
...
- Repository fixture `fixtures/release/runtime-smoke-evidence.json` records the v77 SMOKEJSON real-read command target, Node v193 consumption boundary, forbidden command list, failure taxonomy, and no-write proof without executing LOAD, COMPACT, SETNXEX, or restore

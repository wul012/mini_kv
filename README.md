...
- Runtime `EXPLAINJSON command` explains a command's parser validity, key target, TTL sensitivity, mutation risk, WAL risk, and side effects without executing it
...
1. Add external control-plane smoke notes once Node consumes hardened `EXPLAINJSON` side effects in its operation evidence flow.
2. Consider deeper command explanation fields only after Node proves it needs them for real preflight decisions.

...
Version 59 is a runnable in-memory KV service with injected build version metadata, command catalog metadata, bounded JSON key inventory, digest-aware structured command risk explanation, read-only write-command execution contracts, stable CHECKJSON write/read fixture samples, self-describing INFOJSON/STATSJSON read-only runtime evidence, read-only storage evidence, and indexed restart recovery evidence with restart/replay cost and retention boundaries:
...
- Repository fixture `fixtures/readonly/index.json` groups CHECKJSON, INFOJSON, and STATSJSON read-only samples for Node v156 real-read capture preparation
...
1. Let Node v156 read Java v50 and mini-kv v59 self-describing read-only evidence after the operator explicitly starts both upstream services.

...
Version 51 is a runnable in-memory KV service with injected build version metadata, command catalog metadata, bounded JSON key inventory, digest-aware structured command risk explanation, and read-only write-command execution contracts:
...
1. Add external control-plane smoke notes once Node consumes `CHECKJSON` execution contracts in gate evidence.
2. Consider deeper command execution-contract fields only after Node proves it needs them for real gate verification.

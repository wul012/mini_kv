# mini-kv v800 route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-operator-role-handle-criterion

v800 advances the draft text package review preflight chain to sequence 15. It freezes `fixtures/release/shard-readiness-v799.json` as the source for this step and keeps the rolling SHARDJSON evidence on the disabled draft text package review surface.
...
The core formatter updates `current_draft_text_package_review_preflight_stage_count` to 15. The stage catalog, review criteria, rejection controls, and review profiles now publish the first 15 entries, while `shard_readiness.cpp` rolls `releaseVersion` and status to `route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-operator-role-handle-criterion-read-only`.

The frozen source fixture for this version is `fixtures/release/shard-readiness-v799.json`. The rolling fixture is regenerated from `SHARDJSON` and carries `releaseVersion=v800`.
...
One-sentence value: v800 adds a versioned review-preflight checkpoint for downstream package review while keeping mini-kv strictly read-only and non-authoritative.

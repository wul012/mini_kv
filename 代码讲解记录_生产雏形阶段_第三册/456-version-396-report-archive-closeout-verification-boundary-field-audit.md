# mini-kv v396 - Archive Closeout Verification Boundary Field Audit

## Goal and Boundary

v396 freezes ixtures/release/shard-readiness-v395.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification chain to $(@{Version=396; Sequence=16; Stage=route-preview-verification-report-archive-closeout-verification-boundary-field-audit; Short=boundary-field-audit; Title=Archive Closeout Verification Boundary Field Audit; Scope=audits boundary fields for readOnly, executionAllowed, router, write, WAL, and service-start semantics}.Stage), using the completed v361-v380 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTJSON closeout chain as immutable source evidence.
...
The version archive under e/396/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v396 makes the completed archive closeout chain easier for Node and Java to consume as versioned, read-only evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
# mini-kv v390 - Archive Closeout Verification Retention Map

## Goal and Boundary

v390 freezes ixtures/release/shard-readiness-v389.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification chain to $(@{Version=390; Sequence=10; Stage=route-preview-verification-report-archive-closeout-verification-retention-map; Short=retention-map; Title=Archive Closeout Verification Retention Map; Scope=maps retained fixtures, screenshots, explanations, and walkthroughs for the verification chain}.Stage), using the completed v361-v380 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTJSON closeout chain as immutable source evidence.
...
The version archive under e/390/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v390 makes the completed archive closeout chain easier for Node and Java to consume as versioned, read-only evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
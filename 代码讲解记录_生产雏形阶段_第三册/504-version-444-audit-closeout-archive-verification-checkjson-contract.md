# mini-kv v444 - Audit Closeout Archive Verification CHECKJSON Contract

## Version Goal

v444 advances the read-only archive verification chain for the closed v421-v440 audit closeout package to stage 4, `route-preview-verification-report-archive-closeout-verification-audit-closeout-archive-verification-checkjson-contract-read-only`. It is not a router, not a write path, not a WAL operation, not a filesystem archive crawler, not a restore/load/compact entry point, and not Java order authority.
...
- `shard_readiness.cpp` embeds one archive verification section in `SHARDJSON`, updates the release to v444, and includes the archive verification digest marker.
- `fixtures/release/shard-readiness-v443.json` freezes the previous current fixture before the v444 current fixture rolls forward.
...
One sentence summary: v444 adds pins CHECKJSON denial of execution for archive verification evidence while preserving mini-kv's read-only/no-router/no-write/no-execution boundary.
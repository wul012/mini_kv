# mini-kv v413 - Archive Closeout Verification Audit Consumer Handoff Audit

## Goal and Boundary

v413 freezes ixtures/release/shard-readiness-v412.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification audit chain to $(@{Version=413; Sequence=13; Stage=route-preview-verification-report-archive-closeout-verification-audit-consumer-handoff-audit; Short=consumer-handoff-audit; Title=Archive Closeout Verification Audit Consumer Handoff Audit; Scope=audits consumer handoff semantics and keeps live reads blocked}.Stage), using the completed v381-v400 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTVERIFYJSON release package as immutable source evidence.
...
The version archive under e/413/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v413 makes the v381-v400 verification release package easier for Node and Java to consume as versioned, read-only audit evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
# mini-kv v787 route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-instruction-preflight-digest-criterion

v787 advances the draft text package review preflight chain to sequence 2. It freezes `fixtures/release/shard-readiness-v786.json` as the source for this step and keeps the rolling SHARDJSON evidence on the disabled draft text package review surface.
...
The core formatter updates `current_draft_text_package_review_preflight_stage_count` to 2. The stage catalog, review criteria, rejection controls, and review profiles now publish the first 2 entries, while `shard_readiness.cpp` rolls `releaseVersion` and status to `route-preview-value-supply-signed-approval-capture-artifact-draft-text-package-review-preflight-instruction-preflight-digest-criterion-read-only`.

The frozen source fixture for this version is `fixtures/release/shard-readiness-v786.json`. The rolling fixture is regenerated from `SHARDJSON` and carries `releaseVersion=v787`.
...
One-sentence value: v787 adds a versioned review-preflight checkpoint for downstream package review while keeping mini-kv strictly read-only and non-authoritative.

# mini-kv v469 Route Preview Archive Maintenance CLI Smoke Contract

v469 advances the read-only route preview archive maintenance ledger to $(System.Collections.Hashtable.Stage). It is not a router, not a write path, not a WAL writer, not a restore/load/compact entry point, and not Java order authority. The version exists so downstream consumers can understand the v441-v460 archive verification package while mini-kv keeps the runtime boundary closed.
...
One sentence summary: v469 adds $(System.Collections.Hashtable.Scope) while preserving mini-kv's read-only/no-router/no-write/no-execution boundary.
# mini-kv v764 route-preview-value-supply-signed-approval-capture-artifact-draft-instruction-preflight-review-digest-slot

v764 advances the draft instruction preflight chain to sequence 4. It freezes `fixtures/release/shard-readiness-v763.json` as the source for this step and keeps the rolling SHARDJSON evidence on the disabled instruction-preflight surface.
...
The core formatter updates `current_instruction_preflight_stage_count` to 4, so the stage catalog, instruction slots, guards, and profiles publish the first 4 entries. `shard_readiness.cpp` updates the rolling release and status to `route-preview-value-supply-signed-approval-capture-artifact-draft-instruction-preflight-review-digest-slot-read-only` and keeps the instruction preflight object embedded exactly once in SHARDJSON.

The frozen source fixture for this version is `fixtures/release/shard-readiness-v763.json`. The current fixture is regenerated from CLI output and carries `releaseVersion=v764`.
...
One-sentence value: v764 thickens the instruction preflight evidence chain while preserving the same no-write/no-execution mini-kv boundary.

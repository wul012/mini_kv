# mini-kv v384 - Archive Closeout Verification Checkjson Contract

## Goal and Boundary

v384 freezes ixtures/release/shard-readiness-v383.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification chain to $(@{Version=384; Sequence=4; Stage=route-preview-verification-report-archive-closeout-verification-checkjson-contract; Short=checkjson-contract; Title=Archive Closeout Verification Checkjson Contract; Scope=pins CHECKJSON semantics for the archive closeout verification command as read-only and non-executable}.Stage), using the completed v361-v380 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTJSON closeout chain as immutable source evidence.
...
The version archive under e/384/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v384 makes the completed archive closeout chain easier for Node and Java to consume as versioned, read-only evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
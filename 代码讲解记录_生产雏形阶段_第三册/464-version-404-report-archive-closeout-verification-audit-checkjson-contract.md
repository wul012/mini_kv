# mini-kv v404 - Archive Closeout Verification Audit Checkjson Contract

## Goal and Boundary

v404 freezes ixtures/release/shard-readiness-v403.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification audit chain to $(@{Version=404; Sequence=4; Stage=route-preview-verification-report-archive-closeout-verification-audit-checkjson-contract; Short=checkjson-contract; Title=Archive Closeout Verification Audit Checkjson Contract; Scope=pins CHECKJSON semantics for the audit command as read-only and non-executable}.Stage), using the completed v381-v400 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTVERIFYJSON release package as immutable source evidence.
...
The version archive under e/404/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v404 makes the v381-v400 verification release package easier for Node and Java to consume as versioned, read-only audit evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
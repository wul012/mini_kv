# mini-kv v775 route-preview-value-supply-signed-approval-capture-artifact-draft-instruction-preflight-source-snippet-slot

v775 advances the draft instruction preflight chain to sequence 15. It freezes `fixtures/release/shard-readiness-v774.json` as the source for this step and keeps the rolling SHARDJSON evidence on the disabled instruction-preflight surface.
...
The core formatter updates `current_instruction_preflight_stage_count` to 15, so the stage catalog, instruction slots, guards, and profiles publish the first 15 entries. `shard_readiness.cpp` updates the rolling release and status to `route-preview-value-supply-signed-approval-capture-artifact-draft-instruction-preflight-source-snippet-slot-read-only` and keeps the instruction preflight object embedded exactly once in SHARDJSON.

The frozen source fixture for this version is `fixtures/release/shard-readiness-v774.json`. The current fixture is regenerated from CLI output and carries `releaseVersion=v775`.
...
One-sentence value: v775 thickens the instruction preflight evidence chain while preserving the same no-write/no-execution mini-kv boundary.

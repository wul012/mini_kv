# mini-kv v406 - Archive Closeout Verification Audit TCP Smoke Contract

## Goal and Boundary

v406 freezes ixtures/release/shard-readiness-v405.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification audit chain to $(@{Version=406; Sequence=6; Stage=route-preview-verification-report-archive-closeout-verification-audit-tcp-smoke-contract; Short=tcp-smoke-contract; Title=Archive Closeout Verification Audit TCP Smoke Contract; Scope=records TCP smoke expectations for audit evidence while stopping the owned server after validation}.Stage), using the completed v381-v400 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTVERIFYJSON release package as immutable source evidence.
...
The version archive under e/406/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v406 makes the v381-v400 verification release package easier for Node and Java to consume as versioned, read-only audit evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
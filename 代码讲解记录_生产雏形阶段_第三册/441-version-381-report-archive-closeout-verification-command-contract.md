# mini-kv v381 - Archive Closeout Verification Command Contract

## Goal and Boundary

v381 freezes ixtures/release/shard-readiness-v380.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification chain to $(@{Version=381; Sequence=1; Stage=route-preview-verification-report-archive-closeout-verification-command-contract; Short=command-contract; Title=Archive Closeout Verification Command Contract; Scope=publishes a read-only verification command for the v361-v380 archive closeout evidence chain}.Stage), using the completed v361-v380 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTJSON closeout chain as immutable source evidence.
...
The version archive under e/381/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v381 makes the completed archive closeout chain easier for Node and Java to consume as versioned, read-only evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
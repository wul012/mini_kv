# mini-kv v561 route-preview-value-draft-slot-template-split

## Goal

v561 advances the read-only `SHARDROUTEVALUEDRAFTJSON` operator evidence value draft chain. It starts the value draft package and splits the 25 draft slot templates into their own helper so the top-level evidence module does not become a catch-all file. The source is the frozen v560 `SHARDROUTEIMPORTPREFLIGHTJSON` package, not a live Node, Java, or mini-kv runtime.

This is not a new execution entry, not a value importer, not write routing, not an active shard router, and not a shortcut to restore, load, compact, or service execution. Control-plane consumers should read it as versioned placeholder evidence only: the actual operator value is still absent.
...
v561 moves the operator evidence value draft one version forward while keeping real values, import, writes, routing, and execution outside mini-kv.

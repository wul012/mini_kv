# mini-kv v446 - Audit Closeout Archive Verification TCP Smoke Contract

## Version Goal

v446 advances the read-only archive verification chain for the closed v421-v440 audit closeout package to stage 6, `route-preview-verification-report-archive-closeout-verification-audit-closeout-archive-verification-tcp-smoke-contract-read-only`. It is not a router, not a write path, not a WAL operation, not a filesystem archive crawler, not a restore/load/compact entry point, and not Java order authority.
...
- `shard_readiness.cpp` embeds one archive verification section in `SHARDJSON`, updates the release to v446, and includes the archive verification digest marker.
- `fixtures/release/shard-readiness-v445.json` freezes the previous current fixture before the v446 current fixture rolls forward.
...
One sentence summary: v446 adds records TCP smoke expectations and owned server cleanup while preserving mini-kv's read-only/no-router/no-write/no-execution boundary.
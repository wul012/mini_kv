# mini-kv v394 - Archive Closeout Verification Java Echo Handoff

## Goal and Boundary

v394 freezes ixtures/release/shard-readiness-v393.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification chain to $(@{Version=394; Sequence=14; Stage=route-preview-verification-report-archive-closeout-verification-java-echo-handoff; Short=java-echo-handoff; Title=Archive Closeout Verification Java Echo Handoff; Scope=records Java echo handoff semantics without giving mini-kv order authority}.Stage), using the completed v361-v380 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTJSON closeout chain as immutable source evidence.
...
The version archive under e/394/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v394 makes the completed archive closeout chain easier for Node and Java to consume as versioned, read-only evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
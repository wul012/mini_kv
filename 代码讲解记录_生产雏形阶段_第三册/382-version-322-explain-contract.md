# mini-kv v322 Route Preview Verification Report Explain Contract

v322 freezes v321 route preview verification report evidence and advances current SHARDJSON to route-preview-verification-report-explain-contract. It is not a route executor, active shard router, write-routing path, WAL writer, service-start approval, restore/load/compact path, credential reader, or deployment control.
...
fixtures/release/shard-readiness-v321.json captures the previous current SHARDJSON output. The post-closeout descriptor records sourceFrozenReleaseVersion=v321, digest fnv1a64:2193962195ba633a, and continuity stage feature-read-only-shard-route-preview-verification-report-explain-contract.

## Report Rollout

src/shard_route_preview_verification_report.cpp moves current_report_stage_count to 3. The shardRoutePreviewVerificationReport section keeps reportedCommand=SHARDROUTEVERIFYJSON, previewCommand=SHARDROUTEJSON, rolloutStage=route-preview-verification-report-explain-contract, rolloutStageSequence=3, rolloutReleaseVersion=v322, and publishedStageCount=3.
...
src/shard_readiness_history.cpp rolls nodeConsumer to v322 report explain-contract evidence and points historical fallback at the frozen v321 fixture.
...
One-line summary: v322 advances the report rollout one named, versioned step while preserving no router, no write, no WAL, and no execution boundaries.
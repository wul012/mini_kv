# mini-kv v864 compared package evidence intake audit

## Goal

v864 publishes compared package evidence intake audit stage 4 of 20: $stage. The version freezes v863 as `fixtures/release/shard-readiness-v863.json` before rolling current evidence to v864.

The latest Node plan read for this version was `D:\nodeproj\orderops-node\docs\plans3\v1351-controlled-read-only-shard-preview-operator-evidence-value-supply-signed-approval-capture-artifact-draft-text-package-compared-evidence-evaluation-preflight-closeout-roadmap.md`. That plan is compatible with this mini-kv step because it treats Java and mini-kv as read-only downstream context and forbids creating compared package evidence, parsing package material, accepting synthetic evidence, capturing approval grants, importing runtime payloads, starting sibling services, or mutating Java/mini-kv state.
...
One-line value: v864 turns the future compared-package intake surface into versioned read-only evidence without making mini-kv an acceptance authority.
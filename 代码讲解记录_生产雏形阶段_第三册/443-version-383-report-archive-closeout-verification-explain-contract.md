# mini-kv v383 - Archive Closeout Verification Explain Contract

## Goal and Boundary

v383 freezes ixtures/release/shard-readiness-v382.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification chain to $(@{Version=383; Sequence=3; Stage=route-preview-verification-report-archive-closeout-verification-explain-contract; Short=explain-contract; Title=Archive Closeout Verification Explain Contract; Scope=pins EXPLAINJSON semantics for the archive closeout verification command as metadata-only evidence}.Stage), using the completed v361-v380 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTJSON closeout chain as immutable source evidence.
...
The version archive under e/383/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v383 makes the completed archive closeout chain easier for Node and Java to consume as versioned, read-only evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
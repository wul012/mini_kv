# mini-kv v407 - Archive Closeout Verification Audit Fixture Freeze

## Goal and Boundary

v407 freezes ixtures/release/shard-readiness-v406.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification audit chain to $(@{Version=407; Sequence=7; Stage=route-preview-verification-report-archive-closeout-verification-audit-fixture-freeze; Short=fixture-freeze; Title=Archive Closeout Verification Audit Fixture Freeze; Scope=freezes the previous shard readiness fixture before rolling current audit evidence forward}.Stage), using the completed v381-v400 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTVERIFYJSON release package as immutable source evidence.
...
The version archive under e/407/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v407 makes the v381-v400 verification release package easier for Node and Java to consume as versioned, read-only audit evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
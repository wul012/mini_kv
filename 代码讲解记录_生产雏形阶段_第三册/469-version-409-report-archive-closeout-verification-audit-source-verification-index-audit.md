# mini-kv v409 - Archive Closeout Verification Audit Source Verification Index Audit

## Goal and Boundary

v409 freezes ixtures/release/shard-readiness-v408.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification audit chain to $(@{Version=409; Sequence=9; Stage=route-preview-verification-report-archive-closeout-verification-audit-source-verification-index-audit; Short=source-verification-index-audit; Title=Archive Closeout Verification Audit Source Verification Index Audit; Scope=audits source verification index continuity without filesystem archive walking or router activation}.Stage), using the completed v381-v400 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTVERIFYJSON release package as immutable source evidence.
...
The version archive under e/409/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v409 makes the v381-v400 verification release package easier for Node and Java to consume as versioned, read-only audit evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
# mini-kv v325 Route Preview Verification Report Tcp Smoke Contract

v325 freezes v324 route preview verification report evidence and advances current SHARDJSON to route-preview-verification-report-tcp-smoke-contract. It is not a route executor, active shard router, write-routing path, WAL writer, service-start approval, restore/load/compact path, credential reader, or deployment control.
...
fixtures/release/shard-readiness-v324.json captures the previous current SHARDJSON output. The post-closeout descriptor records sourceFrozenReleaseVersion=v324, digest fnv1a64:2193962195ba633a, and continuity stage feature-read-only-shard-route-preview-verification-report-tcp-smoke-contract.

## Report Rollout

src/shard_route_preview_verification_report.cpp moves current_report_stage_count to 6. The shardRoutePreviewVerificationReport section keeps reportedCommand=SHARDROUTEVERIFYJSON, previewCommand=SHARDROUTEJSON, rolloutStage=route-preview-verification-report-tcp-smoke-contract, rolloutStageSequence=6, rolloutReleaseVersion=v325, and publishedStageCount=6.
...
src/shard_readiness_history.cpp rolls nodeConsumer to v325 report tcp-smoke-contract evidence and points historical fallback at the frozen v324 fixture.
...
One-line summary: v325 advances the report rollout one named, versioned step while preserving no router, no write, no WAL, and no execution boundaries.
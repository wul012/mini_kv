# mini-kv v400 - Archive Closeout Verification Release Package

## Goal and Boundary

v400 freezes ixtures/release/shard-readiness-v399.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification chain to $(@{Version=400; Sequence=20; Stage=route-preview-verification-report-archive-closeout-verification-release-package; Short=release-package; Title=Archive Closeout Verification Release Package; Scope=packages final v381-v400 archive closeout verification evidence for read-only downstream consumption}.Stage), using the completed v361-v380 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTJSON closeout chain as immutable source evidence.
...
The version archive under e/400/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v400 makes the completed archive closeout chain easier for Node and Java to consume as versioned, read-only evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
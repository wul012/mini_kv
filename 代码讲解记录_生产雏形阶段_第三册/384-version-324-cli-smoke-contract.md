# mini-kv v324 Route Preview Verification Report Cli Smoke Contract

v324 freezes v323 route preview verification report evidence and advances current SHARDJSON to route-preview-verification-report-cli-smoke-contract. It is not a route executor, active shard router, write-routing path, WAL writer, service-start approval, restore/load/compact path, credential reader, or deployment control.
...
fixtures/release/shard-readiness-v323.json captures the previous current SHARDJSON output. The post-closeout descriptor records sourceFrozenReleaseVersion=v323, digest fnv1a64:2193962195ba633a, and continuity stage feature-read-only-shard-route-preview-verification-report-cli-smoke-contract.

## Report Rollout

src/shard_route_preview_verification_report.cpp moves current_report_stage_count to 5. The shardRoutePreviewVerificationReport section keeps reportedCommand=SHARDROUTEVERIFYJSON, previewCommand=SHARDROUTEJSON, rolloutStage=route-preview-verification-report-cli-smoke-contract, rolloutStageSequence=5, rolloutReleaseVersion=v324, and publishedStageCount=5.
...
src/shard_readiness_history.cpp rolls nodeConsumer to v324 report cli-smoke-contract evidence and points historical fallback at the frozen v323 fixture.
...
One-line summary: v324 advances the report rollout one named, versioned step while preserving no router, no write, no WAL, and no execution boundaries.
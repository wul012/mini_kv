# mini-kv v585 route-preview-value-draft-release-package

## Goal

v585 advances the read-only SHARDROUTEVALUEDRAFTJSON operator evidence value draft chain: packages final v561-v585 value draft evidence for read-only downstream consumption. It consumes the frozen v560 SHARDROUTEIMPORTPREFLIGHTJSON package as source evidence, not a live Node, Java, or mini-kv runtime.
...
v585 moves the operator evidence value draft one version forward while keeping real values, import, writes, routing, and execution outside mini-kv.

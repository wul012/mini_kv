# mini-kv v408 - Archive Closeout Verification Audit Source Verification Index

## Goal and Boundary

v408 freezes ixtures/release/shard-readiness-v407.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification audit chain to $(@{Version=408; Sequence=8; Stage=route-preview-verification-report-archive-closeout-verification-audit-source-verification-index; Short=source-verification-index; Title=Archive Closeout Verification Audit Source Verification Index; Scope=indexes the closed v381-v400 source verification range for downstream read-only consumers}.Stage), using the completed v381-v400 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTVERIFYJSON release package as immutable source evidence.
...
The version archive under e/408/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v408 makes the v381-v400 verification release package easier for Node and Java to consume as versioned, read-only audit evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
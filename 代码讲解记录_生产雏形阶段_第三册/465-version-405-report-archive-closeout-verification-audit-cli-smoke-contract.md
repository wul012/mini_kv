# mini-kv v405 - Archive Closeout Verification Audit CLI Smoke Contract

## Goal and Boundary

v405 freezes ixtures/release/shard-readiness-v404.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification audit chain to $(@{Version=405; Sequence=5; Stage=route-preview-verification-report-archive-closeout-verification-audit-cli-smoke-contract; Short=cli-smoke-contract; Title=Archive Closeout Verification Audit CLI Smoke Contract; Scope=records CLI smoke expectations for audit evidence without opening write routing}.Stage), using the completed v381-v400 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTVERIFYJSON release package as immutable source evidence.
...
The version archive under e/405/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v405 makes the v381-v400 verification release package easier for Node and Java to consume as versioned, read-only audit evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
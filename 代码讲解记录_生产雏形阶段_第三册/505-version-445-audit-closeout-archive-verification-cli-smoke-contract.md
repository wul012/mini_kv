# mini-kv v445 - Audit Closeout Archive Verification CLI Smoke Contract

## Version Goal

v445 advances the read-only archive verification chain for the closed v421-v440 audit closeout package to stage 5, `route-preview-verification-report-archive-closeout-verification-audit-closeout-archive-verification-cli-smoke-contract-read-only`. It is not a router, not a write path, not a WAL operation, not a filesystem archive crawler, not a restore/load/compact entry point, and not Java order authority.
...
- `shard_readiness.cpp` embeds one archive verification section in `SHARDJSON`, updates the release to v445, and includes the archive verification digest marker.
- `fixtures/release/shard-readiness-v444.json` freezes the previous current fixture before the v445 current fixture rolls forward.
...
One sentence summary: v445 adds records CLI smoke expectations for reading archive verification evidence while preserving mini-kv's read-only/no-router/no-write/no-execution boundary.
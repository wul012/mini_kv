# mini-kv v386 - Archive Closeout Verification TCP Smoke Contract

## Goal and Boundary

v386 freezes ixtures/release/shard-readiness-v385.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification chain to $(@{Version=386; Sequence=6; Stage=route-preview-verification-report-archive-closeout-verification-tcp-smoke-contract; Short=tcp-smoke-contract; Title=Archive Closeout Verification TCP Smoke Contract; Scope=records TCP smoke expectations for archive closeout verification while stopping the owned server after validation}.Stage), using the completed v361-v380 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTJSON closeout chain as immutable source evidence.
...
The version archive under e/386/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v386 makes the completed archive closeout chain easier for Node and Java to consume as versioned, read-only evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
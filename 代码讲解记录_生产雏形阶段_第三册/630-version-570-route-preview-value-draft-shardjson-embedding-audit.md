# mini-kv v570 route-preview-value-draft-shardjson-embedding-audit

## Goal

v570 advances the read-only SHARDROUTEVALUEDRAFTJSON operator evidence value draft chain: audits that SHARDJSON carries exactly one value draft section. It consumes the frozen v560 SHARDROUTEIMPORTPREFLIGHTJSON package as source evidence, not a live Node, Java, or mini-kv runtime.
...
v570 moves the operator evidence value draft one version forward while keeping real values, import, writes, routing, and execution outside mini-kv.

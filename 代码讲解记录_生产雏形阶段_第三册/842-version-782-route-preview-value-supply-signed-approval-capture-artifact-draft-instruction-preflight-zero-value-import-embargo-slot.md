...
v782 advances the draft instruction preflight chain to sequence 22: $(System.Collections.Hashtable.slug). It freezes ixtures/release/shard-readiness-v781.json as the source for this step and keeps the rolling SHARDJSON evidence on the same disabled instruction-preflight surface.
...
The core formatter updates current_instruction_preflight_stage_count to 22, so the stage catalog, instruction slots, guards, and profiles now publish the first 22 entries. shard_readiness.cpp updates the rolling release and status to $(System.Collections.Hashtable.stage) and keeps the instruction preflight object embedded exactly once in SHARDJSON.

The frozen source fixture for this version is ixtures/release/shard-readiness-v781.json; the current fixture is regenerated from the CLI output and carries eleaseVersion=v782.

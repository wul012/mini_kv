# mini-kv v579 route-preview-value-draft-no-synthetic-evidence-audit

## Goal

v579 advances the read-only SHARDROUTEVALUEDRAFTJSON operator evidence value draft chain: audits that synthetic evidence is rejected, not normalized into data. It consumes the frozen v560 SHARDROUTEIMPORTPREFLIGHTJSON package as source evidence, not a live Node, Java, or mini-kv runtime.
...
v579 moves the operator evidence value draft one version forward while keeping real values, import, writes, routing, and execution outside mini-kv.

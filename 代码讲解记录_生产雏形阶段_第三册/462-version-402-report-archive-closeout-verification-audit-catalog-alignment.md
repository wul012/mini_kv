# mini-kv v402 - Archive Closeout Verification Audit Catalog Alignment

## Goal and Boundary

v402 freezes ixtures/release/shard-readiness-v401.json before rolling the current SHARDJSON fixture forward. The version advances the archive closeout verification audit chain to $(@{Version=402; Sequence=2; Stage=route-preview-verification-report-archive-closeout-verification-audit-catalog-alignment; Short=catalog-alignment; Title=Archive Closeout Verification Audit Catalog Alignment; Scope=aligns the audit command with COMMANDS, COMMANDSJSON, EXPLAINJSON, and CHECKJSON without changing runtime behavior}.Stage), using the completed v381-v400 SHARDROUTEVERIFYREPORTARCHIVECLOSEOUTVERIFYJSON release package as immutable source evidence.
...
The version archive under e/402/ captures configure/build, targeted tests, full CTest, CLI smoke, TCP smoke, and git diff --check. The TCP smoke starts an owned loopback server only for validation and stops it before the version is committed.

## One-Line Value

v402 makes the v381-v400 verification release package easier for Node and Java to consume as versioned, read-only audit evidence while keeping mini-kv firmly outside routing, writing, service startup, and order authority.
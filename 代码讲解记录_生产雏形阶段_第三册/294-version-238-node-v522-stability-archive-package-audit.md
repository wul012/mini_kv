...
This version freezes v237 as ixtures/release/shard-readiness-v237.json and advances the rolling SHARDJSON fixture to v238. The source plan is Node v522, which closed the Java / mini-kv route catalog cleanup twenty-version run and left Java and mini-kv on a recommended-parallel path.
...
The important contract fields remain eadOnly=true, executionAllowed=false, outerActivationAllowed=false, and writeRoutingAllowed=false. sourceFrozenReleaseVersion=v237 tells consumers which immutable fixture to trust. stageSequence=38 and 	rackedPostCloseoutReleaseCount=38 show monotonic continuity from the v201 post-closeout start. 

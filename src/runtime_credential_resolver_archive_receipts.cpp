...
constexpr std::string_view archive_source_contract = "Node v264 credential resolver test-only shell contract";
...
constexpr std::string_view archive_state = "credential-resolver-fake-shell-archive-verification-ready";
...
constexpr std::string_view archive_boundary =
    "credential resolver fake-shell archive non-participation receipt only; mini-kv echoes Node v266 archive "
    "verification boundaries, reads no archive files at runtime, does not rerun fake resolver shell behavior, does not "
    "instantiate a resolver client or secret provider, does not accept or read credential values, does not parse raw "
    "endpoint URLs, does not send external requests, does not connect managed audit, does not write storage or "
    "production records, does not execute schema migration or restore/load/compact/SETNXEX, and does not become a "
    "managed audit storage backend";
constexpr std::string_view archive_node_action =
    "verify mini-kv non-participation with Node v266 credential resolver fake-shell archive verification before Node "
    "v267 upstream echo verification";
...
void append_common_archive_boundary_fields(std::vector<OrderedJsonField>& fields) {
    fields.insert(fields.end(), {
                                    {"read_only", json_bool(true)},
                                    {"execution_allowed", json_bool(false)},
                                    {"archive_verification_only", json_bool(true)},
                                    {"archive_files_read_by_mini_kv", json_bool(false)},
                                    {"archive_verification_reruns_fake_shell_behavior", json_bool(false)},
                                    ...
                                    {"external_audit_service_auto_start_allowed", json_bool(false)},
                                    {"connection_execution_allowed", json_bool(false)},
                                    ...
                                    {"sandbox_managed_audit_state_write_allowed", json_bool(false)},
                                    {"credential_value_required", json_bool(false)},
                                    ...
                                    {"schema_migration_execution_allowed", json_bool(false)},
...
std::string format_archive_files_json() {
    return "[{\"id\":\"v264-html-archive\",\"path\":\"c/264/"
           "sandbox-endpoint-credential-resolver-test-only-shell-contract-v264.html\",\"exists\":true,\"size_bytes\":"
           "3696,\"digest\":\"dff3ed1303149bc5aa2dfaf72d3fd95a33d9a3ebedf4fc84756e68b17b41696e\"},"
           "{\"id\":\"v264-screenshot\",\"path\":\"c/264/pictures/"
           "sandbox-endpoint-credential-resolver-test-only-shell-contract-v264.png\",\"exists\":true,\"size_bytes\":"
           "32911,\"digest\":\"fbbb87cc823c3a440239a7181ede23f1f040026057d6df44a9c70707c51f023b\"},"
           "{\"id\":\"v264-explanation\",\"path\":\"c/264/explanation/"
           "sandbox-endpoint-credential-resolver-test-only-shell-contract-v264.md\",\"exists\":true,\"size_bytes\":"
           "5121,\"digest\":\"055c6b0efcb7d692fb8ff76423099c642a94b7f7e0b9e9af86d03746c3ae531e\"},"
           "{\"id\":\"v264-code-walkthrough\",\"path\":\"code-explanation-production-prototype/"
           "268-sandbox-endpoint-credential-resolver-test-only-shell-contract-v264.md\",\"exists\":true,\"size_bytes\":"
           "9624,\"digest\":\"01a7ee143deb0c4f7d2a047ecfb04b774dafbde8b9759831cbc7c86d89be3551\"},"
           "{\"id\":\"v265-html-archive\",\"path\":\"c/265/"
           "sandbox-endpoint-credential-resolver-test-only-shell-upstream-echo-verification-v265.html\",\"exists\":"
           "true,\"size_bytes\":12427,\"digest\":\"7b8a37a909bab429510204ffb890e75dd43758b95aa277ca2906838ae482f25f\"},"
           "{\"id\":\"v265-screenshot\",\"path\":\"c/265/pictures/"
           "sandbox-endpoint-credential-resolver-test-only-shell-upstream-echo-verification-v265.png\",\"exists\":true,"
           "\"size_bytes\":34249,\"digest\":\"f8404f959e5b412036ebf567d692a1da0912215a8b6c168417bccdde99fdac74\"},"
           "{\"id\":\"v265-explanation\",\"path\":\"c/265/explanation/"
           "sandbox-endpoint-credential-resolver-test-only-shell-upstream-echo-verification-v265.md\",\"exists\":true,"
           "\"size_bytes\":5787,\"digest\":\"47a2d94560a262ac433b1dc25352e26a881e0d40cc42add279e8061065dd5289\"},"
           "{\"id\":\"v265-code-walkthrough\",\"path\":\"code-explanation-production-prototype/"
           "269-sandbox-endpoint-credential-resolver-test-only-shell-upstream-echo-verification-v265.md\",\"exists\":"
           "true,\"size_bytes\":11180,\"digest\":\"966ed8ba13aed763eefa0569345a3b89963e400cd03bf9e727a871ec88ecb78a\"},"
           "{\"id\":\"active-plan\",\"path\":\"docs/plans/"
           "v263-post-disabled-resolver-echo-roadmap.md\",\"exists\":true,\"size_bytes\":9534,\"digest\":"
           "\"ef914293ce52cc3993af29f5450e8823a4f80542d6eaa0fa54b4ec92db67deb0\"}]";
}
...
std::string
credential_resolver_fake_shell_archive_non_participation_receipt_digest(const std::vector<std::string>& read_commands) {
...
std::string format_credential_resolver_fake_shell_archive_non_participation_receipt_json(
    ...
    const auto source_node_v264 = json_object({
        {"ready", json_bool(true)},
        {"profile_version", json_string("managed-audit-manual-sandbox-connection-sandbox-endpoint-credential-resolver-"
                                        "test-only-shell-contract.v1")},
        {"shell_contract_state", json_string("sandbox-endpoint-credential-resolver-test-only-shell-contract-ready")},
        {"shell_mode", json_string("test-only-fake-resolver-contract")},
        {"resolver_kind", json_string("fake-in-memory")},
        {"request_shape_field_count", json_integer(archive_node_v264_request_shape_field_count)},
        {"response_shape_field_count", json_integer(archive_node_v264_response_shape_field_count)},
        {"failure_mapping_count", json_integer(archive_node_v264_failure_mapping_count)},
        {"guard_condition_count", json_integer(archive_node_v264_guard_condition_count)},
        {"fake_resolver_only", json_bool(true)},
        {"handle_only_request", json_bool(true)},
        {"credential_resolver_execution_allowed", json_bool(false)},
    ...
    const auto source_node_v265 = json_object({
        {"ready", json_bool(true)},
        {"profile_version",
         json_string("managed-audit-manual-sandbox-connection-sandbox-endpoint-credential-resolver-test-only-shell-"
                     "upstream-echo-verification.v1")},
        {"verification_state",
         json_string("sandbox-endpoint-credential-resolver-test-only-shell-upstream-echo-verification-ready")},
        {"verification_mode", json_string(archive_source_v265_mode)},
        {"source_span", json_string(archive_source_span)},
        {"source_node_v264_ready", json_bool(true)},
        {"java_v107_echo_ready", json_bool(true)},
        {"mini_kv_v116_non_participation_ready", json_bool(true)},
        {"java_v109_optimization_context_ready", json_bool(true)},
        {"request_shape_field_count", json_integer(archive_node_v264_request_shape_field_count)},
        {"response_shape_field_count", json_integer(archive_node_v264_response_shape_field_count)},
        {"failure_mapping_count", json_integer(archive_node_v264_failure_mapping_count)},
        {"guard_condition_count", json_integer(archive_node_v264_guard_condition_count)},
        {"check_count", json_integer(archive_node_v265_check_count)},
        {"passed_check_count", json_integer(archive_node_v265_check_count)},
    ...
    const auto archived_evidence = json_object({
        {"archive_roots", json_array({json_string("c/264/"), json_string("c/265/")})},
        {"source_versions", json_array({json_string("Node v264"), json_string("Node v265")})},
        {"archive_file_count", json_integer(archive_file_count)},
        {"required_snippet_count", json_integer(archive_required_snippet_count)},
        {"matched_snippet_count", json_integer(archive_required_snippet_count)},
        {"files", format_archive_files_json()},
        {"snippet_groups", format_archive_snippet_groups_json()},
    });
    const auto archive_verification = json_object({
        {"archive_verification_digest", json_string(archive_verification_digest)},
        {"evidence_span", json_string(archive_evidence_span)},
        {"source_node_v264_contract_digest", json_string(archive_node_v264_contract_digest)},
        {"source_node_v265_verification_digest", json_string(archive_node_v265_verification_digest)},
        {"source_node_v264_route_path",
         json_string("/api/v1/audit/managed-audit-manual-sandbox-connection-sandbox-endpoint-credential-resolver-"
                     "test-only-shell-contract")},
        {"source_node_v265_route_path",
         json_string("/api/v1/audit/managed-audit-manual-sandbox-connection-sandbox-endpoint-credential-resolver-"
                     "test-only-shell-upstream-echo-verification")},
        {"archive_verification_reads_files_only", json_bool(true)},
        {"archive_verification_reruns_fake_shell_behavior", json_bool(false)},
        {"upstream_actions_enabled", json_bool(false)},
        {"production_audit_allowed", json_bool(false)},
    });
    const auto summary = json_object({
        {"check_count", json_integer(archive_check_count)},
        {"passed_check_count", json_integer(archive_check_count)},
        {"archive_file_count", json_integer(archive_file_count)},
        {"required_snippet_count", json_integer(archive_required_snippet_count)},
        {"matched_snippet_count", json_integer(archive_required_snippet_count)},
        {"production_blocker_count", json_integer(0)},
        {"warning_count", json_integer(archive_warning_count)},
        {"recommendation_count", json_integer(archive_recommendation_count)},
    });

    std::vector<OrderedJsonField> fields = {
        {"receipt_version", json_string("mini-kv-credential-resolver-fake-shell-archive-non-participation-receipt.v1")},
        {"receipt_fixture_path", json_string(archive_receipt_fixture_path)},
        {"source_archive_verification", json_string(archive_source_verification)},
        {"source_contract", json_string(archive_source_contract)},
        {"source_upstream_echo", json_string(archive_source_upstream_echo)},
        {"consumer_hint", json_string(archive_receipt_consumer)},
        {"current_project_version", json_string(version)},
        {"runtime_project_version", json_string(version)},
        {"current_release_version", json_string(archive_receipt_release_version)},
        {"current_artifact_path_hint", json_string(archive_receipt_artifact_path_hint)},
        ...
        {"runtime_role", json_string(archive_runtime_role)},
        {"source_archive_profile_version", json_string(archive_profile_version)},
        {"source_archive_route_path", json_string(archive_route_path)},
        {"source_archive_verification_state", json_string(archive_state)},
        {"source_ready_for_credential_resolver_fake_shell_archive_verification", json_bool(true)},
        {"source_read_only_archive_verification", json_bool(true)},
        {"source_archive_verification_reruns_fake_shell_behavior", json_bool(false)},
        ...
        {"source_archive_verification_digest", json_string(archive_verification_digest)},
        {"source_evidence_span", json_string(archive_evidence_span)},
        {"source_node_v264_contract_digest", json_string(archive_node_v264_contract_digest)},
        {"source_node_v265_verification_digest", json_string(archive_node_v265_verification_digest)},
        {"source_node_v264", source_node_v264},
        {"source_node_v265", source_node_v265},
        ...
        {"warnings",
         json_array({json_object({
             {"code", json_string("ARCHIVE_VERIFICATION_ONLY")},
             {"severity", json_string("warning")},
             {"message",
              json_string("This profile verifies v264/v265 archive files and route shape only; it does not execute "
                          "fake shell behavior.")},
         })})},
        {"recommendations",
         json_array({
             json_object({{"code", json_string("WRITE_POST_V266_PLAN")}, {"severity", json_string("recommendation")}}),
             json_object({{"code", json_string("KEEP_REAL_RESOLVER_OUT_OF_SCOPE")},
                          {"severity", json_string("recommendation")}}),
         })},
        {"next_required_echo_versions", json_array({json_string("Java v110"), json_string("mini-kv v117")})},
        ...
        {"receipt_digest",
         json_string(credential_resolver_fake_shell_archive_non_participation_receipt_digest(read_commands))},
        {"credential_resolver_fake_shell_archive_non_participation_receipt_only", json_bool(true)},
    };
    append_common_archive_boundary_fields(fields);
    fields.push_back({"boundary", json_string(archive_boundary)});
    fields.push_back({"node_action", json_string(archive_node_action)});

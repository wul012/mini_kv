...
std::string format_closed_boundary_flags_json() {
    ...
    const auto object_json = json_object(fields);
...
std::string format_credential_resolver_signed_human_approval_artifact_contract_non_participation_receipt_json(
    ...
    return "{\"receipt_version\":" + field_string(receipt_version) +
           ...
           ",\"profile_version\":" +
           field_string(node_v314_profile_version) + ",\"contract_state\":" + field_string(node_v314_contract_state) +
           ...
           field_string(node_v314_target_prerequisite_id) +
           ...
           ",\"signed_artifact_contract\":{\"contract_digest\":" +
           field_string(node_v314_contract_digest) + ",\"artifact_name\":" + field_string(node_v314_artifact_name) +
           ...
           ",\"purpose\":" + field_string(node_v314_purpose) + ",\"required_fields\":" + format_required_fields_json() +
           ...
           ",\"blocker_resolved\":\"v312 paused the governance chain with five remaining prerequisites; v314 names the "
           "first concrete missing prerequisite and defines its non-secret contract shape.\""
           ",\"consumer\":\"Java v145 + mini-kv v138, then Node v315\""
           ",\"why_v312_cannot_be_reused\":\"v312 is a closure decision only; it lists signed-human-approval-artifact "
           "as missing but does not define the signed artifact contract fields that upstreams can echo.\""
           ",\"existing_report_reuse_decision\":\"Reuse v312 as source state and v313 as the prerequisite catalog; "
           "create v314 only for the signed-human-approval-artifact contract intake.\""
           ",\"stop_condition\":\"Stop if the contract requires credential values, raw endpoint URLs, provider/client "
           "configuration, HTTP/TCP, runtime shell implementation or invocation, ledger/schema writes, mini-kv "
           "authority, or automatic upstream start.\"}"
           ",\"summary\":{\"check_count\":" +
           ...
           ",\"profile_version\":" +
           field_string(node_v312_profile_version) + ",\"decision_state\":" + field_string(node_v312_decision_state) +
           ",\"ready_for_closure_decision\":true"
           ",\"decision_digest\":" +
           field_string(node_v312_decision_digest) +
           ...
           ",\"remaining_prerequisite_ids\":" +
           format_remaining_prerequisites_json() +
           ...
           ",\"approval_ledger_written\":false"
           ",\"automatic_upstream_start\":false}"
           ",\"mini_kv_receipt\":{\"receipt_mode\":\"signed-human-approval-artifact-contract-non-participation-receipt-"
           "only\""
           ...
           "}"
           ",\"production_blockers\":[]"
           ",\"warnings\":[{\"code\":\"SIGNED_ARTIFACT_CONTRACT_DOES_NOT_CLOSE_ALL_PREREQUISITES\",\"severity\":"
           "\"warning\"}]"
           ",\"recommendations\":[{\"code\":\"RUN_JAVA_V145_AND_MINI_KV_V138_AFTER_V314_ARCHIVE\",\"severity\":"
           "\"recommendation\"},"
           ...
           ",\"recommended_parallel_mini_kv_v138\":\"mini-kv v138 signed human approval artifact contract "
           "non-participation receipt\""
           ",\"next_node_verification\":\"Node v315\"}"
           ",\"next_required_evidence_versions\":[\"Java v145\",\"mini-kv v138\"]"
           ",\"next_actions\":[\"Archive mini-kv v138 as a signed human approval artifact contract non-participation "
           "receipt.\","
           ...
           ",\"receipt_digest\":" +
           field_string(credential_resolver_signed_human_approval_artifact_contract_non_participation_receipt_digest(
               read_commands)) +

...
constexpr std::string_view blocked_source_decision = "Node v268 credential resolver production readiness decision gate";
...
constexpr std::string_view blocked_decision_digest = "2c35360789f4f6755fa66360a669968accfa7135a1f645287203801cd04de702";
constexpr std::string_view blocked_active_plan = "docs/plans/v266-post-fake-shell-archive-roadmap.md";
constexpr std::string_view blocked_next_plan = "docs/plans/v268-post-production-readiness-decision-roadmap.md";
...
constexpr std::string_view blocked_decision_reason =
    "Node v267 proves the fake-shell archive echo chain is aligned, but no approved pre-implementation plan defines "
    "credential handle, endpoint handle, secret provider stub, operator approval, rollback, redaction, simulation, "
    "schema migration, or ledger write boundaries.";
constexpr std::string_view blocked_boundary =
    "credential resolver production-readiness blocked-decision non-participation receipt only; mini-kv echoes Node "
    "v268 blocked decision gate boundaries, does not implement a resolver, does not instantiate a resolver client or "
    "secret provider, does not read credential values, does not parse raw endpoint URLs, does not send external "
    "requests, does not connect managed audit, does not write storage or approval ledger records, does not execute "
    "schema migration or restore/load/compact/SETNXEX, and does not become a managed audit storage backend";
constexpr std::string_view blocked_node_action = "verify mini-kv non-participation with Node v268 blocked production "
                                                 "readiness decision before Node v269 upstream echo verification";
...
void append_common_blocked_boundary_fields(std::vector<OrderedJsonField>& fields) {
    fields.insert(fields.end(), {
                                    {"read_only", json_bool(true)},
                                    {"execution_allowed", json_bool(false)},
                                    {"blocked_decision_only", json_bool(true)},
                                    {"production_readiness_gate_only", json_bool(true)},
                                    ...
                                    {"secret_provider_stub_defined", json_bool(false)},
...
std::string credential_resolver_production_readiness_blocked_decision_non_participation_receipt_digest(
    ...
    return receipt_digest("mini-kv-credential-resolver-production-readiness-blocked-decision-non-participation", parts);
}

std::string format_credential_resolver_production_readiness_blocked_decision_non_participation_receipt_json(
    const std::vector<std::string>& read_commands) {
    const auto source_node_v267 = json_object({
        {"source_version", json_string("Node v267")},
        {"profile_version", json_string(blocked_source_v267_profile_version)},
        {"verification_state", json_string(blocked_source_v267_state)},
        {"ready_for_upstream_echo_verification", json_bool(true)},
        {"verification_digest", json_string(blocked_source_v267_digest)},
        {"source_span", json_string(blocked_source_v267_span)},
        {"source_node_v266_ready", json_bool(true)},
        {"java_v110_echo_ready", json_bool(true)},
        {"mini_kv_v117_non_participation_ready", json_bool(true)},
        {"archive_counts_aligned", json_bool(true)},
        {"archive_snippets_aligned", json_bool(true)},
        {"archive_no_rerun_aligned", json_bool(true)},
        ...
        {"node_v267_blocks_real_resolver", json_bool(true)},
        {"check_count", json_integer(blocked_source_check_count)},
        {"passed_check_count", json_integer(blocked_source_check_count)},
        {"archive_file_count", json_integer(blocked_source_archive_file_count)},
        {"evidence_file_count", json_integer(blocked_source_evidence_file_count)},
        {"required_snippet_count", json_integer(blocked_source_required_snippet_count)},
        {"matched_snippet_count", json_integer(blocked_source_matched_snippet_count)},
    ...
    const auto production_readiness_decision = json_object({
        {"decision_digest", json_string(blocked_decision_digest)},
        {"decision_mode", json_string(blocked_decision_mode)},
        {"source_span", json_string(blocked_source_span)},
        {"decision", json_string("blocked")},
        {"reason", json_string(blocked_decision_reason)},
        {"allows_real_resolver_pre_implementation_plan", json_bool(false)},
        {"allows_real_credential_resolver_implementation", json_bool(false)},
        {"allows_secret_provider_stub", json_bool(false)},
        {"allows_secret_provider_runtime", json_bool(false)},
        ...
        {"next_plan_required_before_implementation", json_bool(true)},
    });
    const auto summary = json_object({
        {"check_count", json_integer(blocked_check_count)},
        {"passed_check_count", json_integer(blocked_passed_check_count)},
        {"source_check_count", json_integer(blocked_source_check_count)},
        {"source_passed_check_count", json_integer(blocked_source_check_count)},
        {"archive_file_count", json_integer(blocked_source_archive_file_count)},
        {"evidence_file_count", json_integer(blocked_source_evidence_file_count)},
        {"required_snippet_count", json_integer(blocked_source_required_snippet_count)},
        {"matched_snippet_count", json_integer(blocked_source_matched_snippet_count)},
        {"missing_pre_implementation_requirement_count", json_integer(blocked_missing_requirement_count)},
        {"production_blocker_count", json_integer(blocked_production_blocker_count)},
        {"warning_count", json_integer(blocked_warning_count)},
        {"recommendation_count", json_integer(blocked_recommendation_count)},
    });
    const auto evidence_endpoints = json_object({
        {"production_readiness_decision_gate_json", json_string(blocked_route_path)},
        {"production_readiness_decision_gate_markdown",
         json_string("/api/v1/audit/managed-audit-manual-sandbox-connection-credential-resolver-production-readiness-"
                     "decision-gate?format=markdown")},
        {"source_node_v267_json",
         json_string("/api/v1/audit/managed-audit-manual-sandbox-connection-credential-resolver-fake-shell-archive-"
                     "upstream-echo-verification")},
        {"source_node_v267_markdown",
         json_string("/api/v1/audit/managed-audit-manual-sandbox-connection-credential-resolver-fake-shell-archive-"
                     "upstream-echo-verification?format=markdown")},
        {"active_plan", json_string(blocked_active_plan)},
        {"next_plan", json_string(blocked_next_plan)},
    });

    std::vector<OrderedJsonField> fields = {
        {"receipt_version",
         json_string("mini-kv-credential-resolver-production-readiness-blocked-decision-non-participation-receipt.v1")},
        {"receipt_fixture_path", json_string(blocked_receipt_fixture_path)},
        {"source_decision_gate", json_string(blocked_source_decision)},
        {"source_upstream_echo", json_string(blocked_source_upstream_echo)},
        {"consumer_hint", json_string(blocked_receipt_consumer)},
        {"current_project_version", json_string(version)},
        {"runtime_project_version", json_string(version)},
        {"current_release_version", json_string(blocked_receipt_release_version)},
        {"current_artifact_path_hint", json_string(blocked_receipt_artifact_path_hint)},
        ...
        {"runtime_role", json_string(blocked_runtime_role)},
        {"source_profile_version", json_string(blocked_profile_version)},
        {"source_route_path", json_string(blocked_route_path)},
        {"source_decision_gate_state", json_string(blocked_decision_gate_state)},
        {"source_readiness_decision", json_string(blocked_readiness_decision)},
        {"source_decision_gate_evaluated", json_bool(true)},
        {"source_production_readiness_gate_only", json_bool(true)},
        {"source_read_only_decision_gate", json_bool(true)},
        {"source_ready_for_credential_resolver_pre_implementation_plan", json_bool(false)},
        ...
        {"source_node_v267", source_node_v267},
        ...
        {"warnings",
         json_array({
             json_object({{"code", json_string("DECISION_GATE_ONLY")}, {"severity", json_string("warning")}}),
             json_object({{"code", json_string("SOURCE_CHAIN_READY_BUT_NOT_PRODUCTION_READY")},
                          {"severity", json_string("warning")}}),
         })},
        {"recommendations",
         json_array({
             json_object({{"code", json_string("WRITE_SUCCESSOR_PLAN")}, {"severity", json_string("recommendation")}}),
             json_object({{"code", json_string("REQUEST_PARALLEL_UPSTREAM_ECHO")},
                          {"severity", json_string("recommendation")}}),
         })},
        {"evidence_endpoints", evidence_endpoints},
        {"next_required_echo_versions", json_array({json_string("Java v111"), json_string("mini-kv v118")})},
        ...
        {"receipt_digest",
         json_string(credential_resolver_production_readiness_blocked_decision_non_participation_receipt_digest(
             read_commands))},
        {"credential_resolver_production_readiness_blocked_decision_non_participation_receipt_only", json_bool(true)},
    };
    append_common_blocked_boundary_fields(fields);
    fields.push_back({"boundary", json_string(blocked_boundary)});
    fields.push_back({"node_action", json_string(blocked_node_action)});

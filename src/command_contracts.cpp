...
bool is_known_command(std::string_view command) {
    return command_catalog::is_known(command);
}

std::string format_commands() {
    std::string response = "command_count=" + std::to_string(command_catalog::count()) + " commands=";
    bool first = true;
    for (const auto& entry : command_catalog::entries()) {
        if (!first) {
            response.push_back(';');
        }
        first = false;

...
namespace minikv::shard_preview_disabled_fake_harness_contract_upstream_echo_verification_split_non_participation_validation {
namespace {
...
std::string format_disabled_fake_harness_contract_upstream_echo_verification_split_non_participation_validation_json(
    int source_release_window_stage_count,
    ...
    const bool source_locked = source_release_window_stage_count == 15 && source_release_window_complete;
    ...
    const bool mini_kv_boundary_closed = !mini_kv_imports_node_modules && !mini_kv_executes_node_checks &&
                                         ...
                                         !mini_kv_reads_credentials && !mini_kv_parses_historical_receipts;
    ...
    return "{\"disabledFakeHarnessContractUpstreamEchoVerificationSplitNonParticipationValidationPassed\":" +
           json_bool(validation_passed) +
           ",\"sourceReleaseWindowReadinessPacketSplitNonParticipationPublishedStageCount\":" +
           std::to_string(source_release_window_stage_count) +
           ",\"sourceReleaseWindowReadinessPacketSplitNonParticipationComplete\":" +
           json_bool(source_release_window_complete) +

...
- 根 `CMakeLists.txt` 有 2,596 行，v1671 时有 315 个产品源和 342 个普通测试注册与策略混排；
  v1672 合法新增一个 dispatch 翻译单元，因此 v1674 Step-0 实际清点为 316。计划初稿写成 315，
  属于算术记录错误；本版以根 CMake 与磁盘集合双向一致的现实值为准，不删源码凑数。
...
验收：316 个产品翻译单元前后集合/顺序一致；未登记、重复和错误扩展名红路径成立；根
`CMakeLists.txt` 硬 ratchet 收紧到不超过 150 行，任何单个新增 CMake 模块不超过 500 行；九分评分表
...
| 产品 ownership 数据化 | v1674 source manifest | 316 项集合/顺序、遗漏/重复/扩展名红路径 | 本地完成 |
| 根构建控制面可读 | 根 CMake + 小模块 | 150 行上限与模块 500 行上限 | 完成：127/257 |
| 外部合同不变 | CLI/fixture/public header | hash/diff、focused/full CTest | 本地 354/354 与真实 smoke 通过 |
| 跨平台成立 | GitHub Actions | v1674 七条 job 全绿 | 以精确提交的远端 run 为最终门 |

## 九分评分表

评分不是代码作者的主观印象，而是“分值、证据、扣分理由”三者同时存在的候选结论。远端
CI 未全绿前，跨平台项计 0 分，总分只有 8.1，不能宣布到点；七条 v1674 job 全绿后才记
1.0 分并达到 9.1。

| 维度 | 候选分 | 机械证据 | 主动扣分 |
|---|---:|---|---|
| 合同安全 | 1.9/2.0 | 产品/fixture diff 为 0、354 CTest、CLI/WAL reopen smoke | 不把教学项目写成生产数据库 |
| 架构职责 | 1.7/2.0 | command table、test manifest、product manifest；根 CMake 127 行 | 仍保留大批历史长名 family |
| 重复与数据建模 | 1.2/1.5 | 62 命令、342 测试、316 源码由数据表达 | 735/883 与 277 个旧长 target 尚未清零 |
| 构建与测试设计 | 1.5/1.5 | 8-shard/legacy、Release assertions、六类 parser 红路径 | 无 |
| 防回流机械门 | 1.0/1.0 | 316 双向集合、150/500 体积门、exact test order、source-size/name ratchet | 无 |
| 跨平台 CI | 1.0/1.0 | 仅以 v1674 七条 job 全绿计分 | 当前 pending 时本项为 0 |
| 文档证据 | 0.4/0.5 | 中文 walkthrough、四张选择性截图、scorecard、可复现命令 | 保留 0.1，防止文档替代外部评审 |
| 范围克制 | 0.4/0.5 | 零产品源/fixture 改动，无新 router/write/network/execution | 0.1 留给后续真实需求变化的可演进性 |

最终候选分：`9.1/10.0`，但只有完成最后 full CTest、真实 smoke、commit/tag/push 且 v1674
七条 CI 全绿后生效。该分数表示当前内部工程优雅度达到本计划的九分门，不表示没有历史
债务，也不替代后续外部 review。

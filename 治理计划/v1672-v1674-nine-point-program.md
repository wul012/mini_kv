...
- 根 `CMakeLists.txt` 有 2,596 行，314 个产品源和 342 个普通测试注册与策略混排。
...
1. 抽象名：`TestCaseManifest`，一行只描述 kind、target、CTest name、source。
...
5. script smoke、standalone test 和 target-specific override 仍显式留在构建模块。
...
验收：314 个产品翻译单元前后集合/顺序一致；未登记、重复和错误扩展名红路径成立；根
...
| 测试身份数据化 | v1673 test manifest | 342 项逐项 oracle、parser 红路径、`ctest -N` diff | 缺失 |
| 产品 ownership 数据化 | v1674 source manifest | 314 项集合/顺序、遗漏/重复/扩展名红路径 | 缺失 |
| 根构建控制面可读 | 根 CMake + 小模块 | 650 行上限与模块 500 行上限 | 缺失 |

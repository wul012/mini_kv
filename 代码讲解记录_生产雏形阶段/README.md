...
- `170-version-114-credential-resolver-non-participation-receipt.md`: v114 adds a read-only credential resolver non-participation receipt for Node v261, echoing Node v260 credential resolver decision fields and no-go conditions while proving mini-kv does not instantiate a secret provider, invoke a resolver, auto-start, write storage, read/load/store/include credential values, parse raw endpoint URLs, send external requests, execute schema migration or LOAD/COMPACT/RESTORE/SETNXEX, or become managed audit storage.
- `169-version-113-sandbox-endpoint-handle-non-participation-receipt.md`: v113 adds a read-only sandbox endpoint handle non-participation receipt for Node v259, echoing Node v258 endpoint-handle preflight review counts and handle/policy/window fields while proving mini-kv does not auto-start, write storage, read credential values, parse raw endpoint URLs, send external requests, execute LOAD/COMPACT/RESTORE/SETNXEX, or become managed audit storage.

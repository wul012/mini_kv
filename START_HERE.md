...
Current focus: **v1673 validated test manifest**. The 342 ordinary linked/source-dir cases now keep phase, kind, private target, public CTest name, and source in one LF-only manifest; a small suite module retains the three standalone tests, nine special smoke/script registrations, and target-specific overrides. The root CMake file falls from 2,597 to 436 lines, while default 8-shard and explicit legacy configurations both produce the exact pre-migration 354-name order. Configure-time validation and architecture red paths reject malformed rows, CRLF, unknown kinds/phases, duplicate identities, and missing sources. Public commands, fixtures, product sources, CTest names/properties, WAL/Snapshot formats, and the independent OSFS layer are unchanged.

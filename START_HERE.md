...
Current focus: **v1672 evidence command table**. Sixty-two no-argument read-only evidence commands now map directly to their existing formatters through one private compile-time table; parameter validation still happens before formatter execution, while stateful STATS/INFO and key-bearing route commands remain explicit `CommandProcessor` branches. `src/command.cpp` falls from 732 to 258 nonblank lines, the before/after 2,093,273-byte CLI oracle is identical, and all bundled, legacy, and standalone test targets explicitly retain assertions even in Release builds. Public commands, fixtures, command responses, WAL/Snapshot formats, test names, and the independent OSFS layer are unchanged.

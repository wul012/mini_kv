...
- CMake builds with 354 registered tests across command, WAL, snapshot, atomic-file, RESP, TCP, OSFS, fixture, archive, and governance evidence suites; ordinary builds link 342 cases through 8 stable runners while instrumented builds retain per-test executables
...
5. 354 registered tests with stable 8-runner default linking plus runtime JSON verification

## Latest version summary

Current focus: **v1674 validated product-source manifest and nine-point gate**. The `minikv` library's 316 translation units now live in one LF-only ordered manifest, while a 62-line loader rejects duplicate, missing, stale, unregistered, wrong-extension, or executable-main entries before target creation. Root `CMakeLists.txt` is 127 physical lines with a 150-line ratchet; every participating CMake module stays below 500 lines. A single tracked test wrapper exports the exact 354-name registration order without parsing generator-specific `CTestTestfile.cmake` syntax, and the architecture scan reuses one in-memory source inventory instead of repeating hundreds of hosted-Windows filesystem probes. Product sources, fixtures, public commands, test identities, WAL/Snapshot formats, and the independent OSFS layer are unchanged; the 9.1 score becomes valid only after all seven v1674 CI jobs are green.
